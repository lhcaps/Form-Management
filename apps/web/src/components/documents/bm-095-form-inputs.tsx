...
/**
 * BM-095 — QĐ huỷ bỏ QĐ khởi tố bị can
 * Stage: KHOI_TO, Group: G02. TT 03/2026-VKSTC, Mẫu số 95/HS.
 *
 * Căn cứ: Điều 154, 156 BLTTHS.
 * Nghiệp vụ: VKS huỷ QĐ khởi tố bị can đã ban hành.
 */
...
const EMPTY_FORM: Bm095Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "95/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
const REQUIRED_FIELDS: ReadonlyArray<[keyof Bm095Form, string, string]> = [
...
function buildIssuePlaceAndDateLine(form: Bm095Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm095Form): string {
  ...
  return `Xét thấy QĐ số ${d.cancelledDocumentCode.trim()}${dateText ? ` ${dateText}` : ""} của ${d.cancelledDocumentAgency.trim()} về việc khởi tố bị can ${d.accusedFullName.trim()} trong vụ án ${d.caseTitle.trim()} về tội "${d.offenseName.trim()}" không có căn cứ và trái pháp luật;`;
}

function normalizeFormInputs(payload: RenderPayload | null): Bm095Form {
  const f = EMPTY_FORM;
  return {
    agency: {
...
function validateForm(form: Bm095Form): string[] {
...
function buildSaveBody(form: Bm095Form) {
...
export function Bm095FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm095Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm095Form, K extends keyof Bm095Form[S]>(
    section: S,
    key: K,
    value: Bm095Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-095 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-095 thành công.");
      await onSaved?.();
    } catch (e) {
