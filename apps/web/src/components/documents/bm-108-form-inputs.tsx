...
/**
 * BM-108 — QĐ huỷ bỏ QĐ tạm đình chỉ VAHS
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 108/HS.
 *
 * Căn cứ: Điều 41, 247 BLTTHS 2015.
 */
...
const EMPTY_FORM: Bm108Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "108/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm108Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm108Form): string {
...
function normalizeFormInputs(payload: RenderPayload | null): Bm108Form {
  const f = EMPTY_FORM;
  return {
    agency: {
...
function lookupValue(form: Bm108Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm108Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm108Form) {
  return {
    agency: {
...
export function Bm108FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm108Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm108Form, K extends keyof Bm108Form[S]>(
    section: S,
    key: K,
    value: Bm108Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-108 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-108 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-108"
        title="Dữ liệu biểu mẫu QĐ huỷ bỏ QĐ tạm đình chỉ VAHS"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-108"}
      ...
      <BmFormSection title="2. Quyết định bị huỷ bỏ" description="Thông tin QĐ tạm đình chỉ VAHS cần huỷ bỏ." requiredCount={2}>
        <BmFieldText
          label="Số QĐ bị huỷ"
          required
