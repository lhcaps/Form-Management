 ...
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 131/HS.
 *
 * Căn cứ: Điều 41, 50 BLTTHS 2015.
 */
...
const EMPTY_FORM: Bm131Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "131/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm131Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm131Form): string {
...
function normalizeFormInputs(payload: RenderPayload | null): Bm131Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    request: {
      procedureArticlesLine: nested(payload, "request.procedureArticlesLine") || "",
      caseTitle: nested(payload, "request.caseTitle") || "",
      propertyDescription: nested(payload, "request.propertyDescription") || "",
      originalAppraisalValue: nested(payload, "request.originalAppraisalValue") || "",
      originalAppraisalDate: nested(payload, "request.originalAppraisalDate") || "",
      originalAppraisalAgency: nested(payload, "request.originalAppraisalAgency") || "",
      reasonForReappraisal: nested(payload, "request.reasonForReappraisal") || "",
    },
    recipients: {
...
function lookupValue(form: Bm131Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm131Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm131Form) {
  return {
    agency: {
    ...
    request: {
      ...
      propertyDescription: form.request.propertyDescription,
      originalAppraisalValue: form.request.originalAppraisalValue,
      originalAppraisalDate: form.request.originalAppraisalDate,
      originalAppraisalAgency: form.request.originalAppraisalAgency,
      reasonForReappraisal: form.request.reasonForReappraisal,
...
export function Bm131FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm131Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm131Form, K extends keyof Bm131Form[S]>(
    section: S,
    key: K,
    value: Bm131Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-131 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-131 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-131"
        title="Dữ liệu biểu mẫu Yêu cầu định giá lại tài sản"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-131"}
      ...
      <BmFormSection title="2. Thông tin định giá lại" description="Nhập thông tin định giá lại tài sản." requiredCount={2}>
        <BmFieldTextarea
        ...
        <BmFieldText
          ...
          required
          value={form.request.propertyDescription}
          onChange={(v) => patch("request", "propertyDescription", v)}
        />
        <BmFieldText
          label="Giá trị định giá trước"
          value={form.request.originalAppraisalValue}
          onChange={(v) => patch("request", "originalAppraisalValue", v)}
        />
        <BmFieldText
          label="Ngày định giá trước"
          value={form.request.originalAppraisalDate}
          onChange={(v) => patch("request", "originalAppraisalDate", v)}
        />
        <BmFieldText
          label="Đơn vị định giá trước"
          value={form.request.originalAppraisalAgency}
          onChange={(v) => patch("request", "originalAppraisalAgency", v)}
        />
        <BmFieldText
          label="Lý do định giá lại"
          required
          value={form.request.reasonForReappraisal}
          onChange={(v) => patch("request", "reasonForReappraisal", v)}
        />
        <BmFieldTextarea
          label="Lý do tự sinh"
          fullWidth
