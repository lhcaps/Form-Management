...
/**
 * BM-120 — QĐ phê chuẩn lệnh bắt bị can để tạm giam
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 120/HS.
 *
...
const EMPTY_FORM: Bm120Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "120/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm120Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm120Form): string {
...
function normalizeFormInputs(payload: RenderPayload | null): Bm120Form {
  const f = EMPTY_FORM;
  return {
    agency: {
...
function lookupValue(form: Bm120Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm120Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm120Form) {
  return {
    agency: {
...
export function Bm120FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm120Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm120Form, K extends keyof Bm120Form[S]>(
    section: S,
    key: K,
    value: Bm120Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-120 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-120 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-120"
        title="Dữ liệu biểu mẫu QĐ phê chuẩn lệnh bắt bị can để tạm giam"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-120"}
      ...
      <BmFormSection title="2. Thông tin lệnh bắt" description="Nhập thông tin lệnh bắt bị can." requiredCount={2}>
        <BmFieldTextarea
