...
/**
 * BM-153 — Quyết định huỷ bỏ QĐ đình chỉ vụ án đối với bị can
 * Stage: TRUY_TO, Group: G05. TT 03/2026-VKSTC, Mẫu số 153/HS.
 *
 * Căn cứ: Căn cứ Điều 41 và Điều 250 của Bộ luật Tố tụng hình sự;
...
const EMPTY_FORM: Bm153Form = {
  ...
  legalBasis: {
  procedureArticlesLine: 'Căn cứ Điều 41 và Điều 250 của Bộ luật Tố tụng hình sự;',
...
const REQUIRED_FIELDS: ReadonlyArray<[string, string]> =   [
    ...
    ["Căn cứ QĐ đình chỉ bị huỷ", "legalBasis.annulledDecisionLine"],
...
function buildIssuePlaceAndDateLine(form: Bm153Form): string {
  return issuePlaceDateLine(form.agency.issuePlace, form.document.issueDateIso);
}

function normalizeFormInputs(payload: RenderPayload | null): Bm153Form {
  const f = EMPTY_FORM;
  if (!payload) return f;
  return {
    agency: {
...
function lookupValue(form: Bm153Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
...
function validateForm(form: Bm153Form): string[] {
  return REQUIRED_FIELDS.filter(([, path]) => !lookupValue(form, path)).map(
    ([label]) => label,
  );
}

function buildSaveBody(form: Bm153Form) {
  return {
    agency: {
...
export function Bm153FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm153Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm153Form, K extends keyof Bm153Form[S]>(
    section: S,
    key: K,
    value: Bm153Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-153 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-153 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-153"
        title="Dữ liệu biểu mẫu Quyết định huỷ bỏ QĐ đình chỉ vụ án đối với bị can"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-153"}
