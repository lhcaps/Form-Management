 ...
 * Stage: KHOI_TO, Group: G02. TT 03/2026-VKSTC, Mẫu số 96/HS.
 *
 * Căn cứ: Điều 36, 37, 167 BLTTHS.
 * Nghiệp vụ: VKS yêu cầu CQĐT ra QĐ khởi tố bị can theo yêu cầu của VKS.
 */
...
const EMPTY_FORM: Bm096Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "96/YCT-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
const REQUIRED_FIELDS: ReadonlyArray<[keyof Bm096Form, string, string]> = [
...
function buildIssuePlaceAndDateLine(form: Bm096Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm096Form): string {
  const r = form.request;
  if (!r.accusedFullName || !r.caseTitle) return "";
  return `Qua xét nội dung vụ án ${r.caseTitle.trim()}, có căn cứ cho rằng ${r.accusedFullName.trim()}${r.accusedDateOfBirth ? `, sinh năm ${cleanText(r.accusedDateOfBirth)}` : ""}${r.accusedAddress ? `, địa chỉ: ${r.accusedAddress.trim()}` : ""} đã thực hiện hành vi phạm tội "${r.offenseName.trim()}" theo quy định của Bộ luật Hình sự.`;
}

function normalizeFormInputs(payload: RenderPayload | null): Bm096Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    request: {
      ...
      accusedFullName:
        ...
        "",
      accusedDateOfBirth:
        nested(payload, "request.accusedDateOfBirth") ||
        nested(payload, "accused.dateOfBirth") ||
        "",
      accusedAddress:
        nested(payload, "request.accusedAddress") || nested(payload, "accused.address") || "",
    },
    recipients: {
...
function validateForm(form: Bm096Form): string[] {
...
function buildSaveBody(form: Bm096Form) {
  ...
  return {
    ...
    request: {
      ...
      accusedDateOfBirth: form.request.accusedDateOfBirth,
      accusedAddress: form.request.accusedAddress,
...
export function Bm096FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm096Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm096Form, K extends keyof Bm096Form[S]>(
    section: S,
    key: K,
    value: Bm096Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-096 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-096 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-096"
        title="Dữ liệu biểu mẫu Yêu cầu ra QĐ khởi tố bị can"
        subtitle="Biểu mẫu TT 03/2026-VKSTC · Mẫu số 96/HS · Stage KHOI_TO (G02). Căn cứ Điều 36, 37, 167 BLTTHS."
      ...
      <BmFormSection
        ...
        description="Nhập thông tin vụ án và bị can cần khởi tố."
      >
        <BmFieldTextarea
        ...
        <BmFieldText
          label="Năm sinh"
          value={form.request.accusedDateOfBirth}
          onChange={(v) => patch("request", "accusedDateOfBirth", v)}
        />
        <BmFieldText
          label="Địa chỉ"
          value={form.request.accusedAddress}
          onChange={(v) => patch("request", "accusedAddress", v)}
        />
        <BmFieldTextarea
