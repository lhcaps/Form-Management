 ...
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 119/HS.
 *
 * Căn cứ: Điều 41, 192 BLTTHS 2015.
 */
...
const EMPTY_FORM: Bm119Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "119/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm119Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm119Form): string {
...
function normalizeFormInputs(payload: RenderPayload | null): Bm119Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    decision: {
      ...
      searchLocation: nested(payload, "decision.searchLocation") || "",
...
function lookupValue(form: Bm119Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm119Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm119Form) {
  return {
    agency: {
    ...
    decision: {
      ...
      searchLocation: form.decision.searchLocation,
...
export function Bm119FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm119Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm119Form, K extends keyof Bm119Form[S]>(
    section: S,
    key: K,
    value: Bm119Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-119 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-119 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-119"
        title="Dữ liệu biểu mẫu QĐ phê chuẩn lệnh khám xét"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-119"}
      ...
      <BmFormSection title="2. Thông tin lệnh khám xét" description="Nhập thông tin lệnh khám xét được phê chuẩn." requiredCount={2}>
        <BmFieldTextarea
        ...
        />
        <BmFieldText
          label="Địa điểm khám xét"
          required
          value={form.decision.searchLocation}
          onChange={(v) => patch("decision", "searchLocation", v)}
        />
        <BmFieldText
          label="Số lệnh khám xét"
          required
          value={form.decision.warrantCode}
          onChange={(v) => patch("decision", "warrantCode", v)}
        />
        <BmFieldDate
          label="Ngày lệnh khám xét"
