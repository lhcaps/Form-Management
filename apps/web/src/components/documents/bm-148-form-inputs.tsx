...
function extractDecisionCodeFromLine(value: unknown): string {
  const raw = text(value).trim();

  if (!raw) return "";

  const match = raw.match(/số\s+(.+?)\s+(?:ngày|của|đối với|về tội|,|;)/iu);

  return match?.[1]?.trim() ?? "";
}

function extractDateFromLegalBasisLine(value: unknown): string {
  const raw = text(value).trim();

  if (!raw) return "";

  const vietnameseDate = raw.match(/ngày\s+(\d{1,2})\s+tháng\s+(\d{1,2})\s+năm\s+(\d{4})/iu);
  if (vietnameseDate) {
    const [, day, month, year] = vietnameseDate;
    return `${year}-${month.padStart(2, "0")}-${day.padStart(2, "0")}`;
  }

  const slashDate = raw.match(/ngày\s+(\d{1,2})\/(\d{1,2})\/(\d{4})/iu) || raw.match(/\b(\d{1,2})\/(\d{1,2})\/(\d{4})\b/u);
  if (slashDate) {
...
function extractOffenseNameFromLine(value: unknown): string {
  const raw = text(value).trim();

  if (!raw) return "";

  const quoted = raw.match(/về tội\s+[“"]([^”"]+)[”"]/iu);
  if (quoted?.[1]) return quoted[1].trim();

  const plain = raw.match(/về tội\s+([^,;]+)/iu);
  return plain?.[1]?.trim() ?? "";
}

function extractLegalClauseFromLine(value: unknown): string {
  const raw = text(value).trim();
  const match = raw.match(/quy định tại\s+(khoản\s+\d+)/iu);

  return match?.[1]?.trim() ?? "";
}

function extractLegalArticleFromLine(value: unknown): string {
  const raw = text(value).trim();
  const match = raw.match(/\b(Điều\s+\d+)\b/iu);
...
function syncDerivedFields(form: Bm148FormInputs): Bm148FormInputs {
  const name = form.person.fullName.trim();
  const offense = form.helper.offenseName.trim();
  const clause = form.helper.legalClause.trim();
  const article = form.helper.legalArticle.trim();
  const currentCaseTitle = form.helper.caseTitle.trim();
  const locationSuffix = currentCaseTitle.match(/\s+tại\s+.+$/iu)?.[0] ?? "";
  const caseTitle = offense ? `Vụ án ${offense}${locationSuffix}` : currentCaseTitle;
  const agency =
    form.helper.investigationAgencyName.trim() ||
    stripRecipientLine(form.recipients.line1) ||
    "Cơ quan Cảnh sát điều tra Công an Thành phố Hồ Chí Minh";

  const syncedBase: Bm148FormInputs = {
    ...form,
    helper: {
      ...form.helper,
      offenseName: form.helper.offenseName,
      legalClause: clause,
      legalArticle: article,
      caseTitle,
      investigationAgencyName: agency,
    },
  };
  ...
  return {
    ...
    document: {
      ...syncedBase.document,
      issuePlaceAndDateLine: syncedBase.document.issueDate
        ? `${syncedBase.agency.issuePlace || "TP. Hồ Chí Minh"}, ${toVietnameseDate(syncedBase.document.issueDate)}`
        : syncedBase.document.issuePlaceAndDateLine,
    },
    helper: {
      ...syncedBase.helper,
      caseTitle,
    ...
    caseDecision: {
      ...syncedBase.caseDecision,
      prosecutionDecisionLegalBasisLine: buildCaseDecisionLine(syncedBase),
    },
    accusedDecision: {
      ...syncedBase.accusedDecision,
      prosecutionDecisionLegalBasisLine: buildAccusedDecisionLine(syncedBase),
    },
    suspension: {
      ...syncedBase.suspension,
      reasonLine: name
        ? ensureSentenceEnd(`Xét thấy cần tạm đình chỉ vụ án hình sự đối với bị can ${name}`, ",")
        : "",
      article1Line: name
        ? ensureSentenceEnd(`Tạm đình chỉ vụ án hình sự đối với bị can ${name} trong ${caseTitle}`, ".")
        : "",
      article2ActionLine:
        syncedBase.suspension.article2ActionLine ||
      ...
      executionRequestLine: ensureSentenceEnd(
        `Yêu cầu ${agency} thực hiện Quyết định này theo quy định của Bộ luật Tố tụng hình sự`,
        "./.",
      ),
    },
    person: {
      ...syncedBase.person,
      birthDateLine: syncedBase.person.dateOfBirth
        ? `Sinh ngày ${toVietnameseDate(syncedBase.person.dateOfBirth).replace(/^ngày\s+/u, "")}${syncedBase.person.placeOfBirth ? ` tại: ${syncedBase.person.placeOfBirth}` : ""}`
        : syncedBase.person.birthDateLine,
      nationalityEthnicityReligionLine: `Quốc tịch: ${syncedBase.person.nationality || "Việt Nam"}; Dân tộc: ${syncedBase.person.ethnicity || "Kinh"}; Tôn giáo: ${syncedBase.person.religion || "Không"}`,
      identityDocumentLine: syncedBase.person.identityNo,
      identityIssueLine: syncedBase.person.identityIssuedDate
        ? `Cấp ngày ${toVietnameseDate(syncedBase.person.identityIssuedDate).replace(/^ngày\s+/u, "")}${syncedBase.person.identityIssuedPlace ? ` Nơi cấp: ${syncedBase.person.identityIssuedPlace}` : ""}`
        : syncedBase.person.identityIssueLine,
    },
    recipients: {
      ...syncedBase.recipients,
      line1: `- ${agency};`,
      line2: name ? `- ${name};` : "",
      archiveLine: syncedBase.recipients.archiveLine || "- Lưu: HSVA, HSKS, VP.",
    },
  };
}
...
function normalizeFormInputs(payload: Record<string, unknown>): Bm148FormInputs {
  ...
  return syncDerivedFields({
    ...
    helper: {
      offenseName: firstText(
        pick(caseInfo, "offenseName"),
        extractOffenseNameFromLine(pick(caseDecision, "prosecutionDecisionLegalBasisLine")),
        extractOffenseNameFromLine(pick(accusedDecision, "prosecutionDecisionLegalBasisLine")),
        "Đánh bạc",
      ),
      legalClause: firstText(
        pick(caseInfo, "legalClause"),
        extractLegalClauseFromLine(pick(caseDecision, "prosecutionDecisionLegalBasisLine")),
        extractLegalClauseFromLine(pick(accusedDecision, "prosecutionDecisionLegalBasisLine")),
        "khoản 1",
      ),
      legalArticle: firstText(
        pick(caseInfo, "legalArticle"),
        extractLegalArticleFromLine(pick(caseDecision, "prosecutionDecisionLegalBasisLine")),
        extractLegalArticleFromLine(pick(accusedDecision, "prosecutionDecisionLegalBasisLine")),
        "Điều 321",
      ),
      caseTitle: firstText(pick(caseInfo, "caseTitle"), "Vụ án đánh bạc tại phường Trung Mỹ Tây"),
      investigationAgencyName,
      caseDecisionCode: firstText(
        pick(caseDecision, "decisionCode"),
        extractDecisionCodeFromLine(pick(caseDecision, "prosecutionDecisionLegalBasisLine")),
        pick(caseInfo, "caseCode"),
        "",
      ),
      caseDecisionDate: firstText(
        toDateInput(caseDecision.decisionDate),
        extractDateFromLegalBasisLine(pick(caseDecision, "prosecutionDecisionLegalBasisLine")),
        "2026-05-06",
      ),
      accusedDecisionCode: firstText(
        pick(accusedDecision, "decisionCode"),
        extractDecisionCodeFromLine(pick(accusedDecision, "prosecutionDecisionLegalBasisLine")),
        pick(caseInfo, "caseCode"),
        "",
      ),
      accusedDecisionDate: firstText(
        toDateInput(accusedDecision.decisionDate),
        extractDateFromLegalBasisLine(pick(accusedDecision, "prosecutionDecisionLegalBasisLine")),
        "2026-05-06",
      ),
    },
    legalBasis: {
...
export function Bm148FormInputsPanel({
  ...
  function prepareFormForSave(current: Bm148FormInputs): Bm148FormInputs {
    const synced = syncDerivedFields(current);

    return {
      ...current,
      document: {
        ...current.document,
        issuePlaceAndDateLine: synced.document.issuePlaceAndDateLine,
      },
      person: {
        ...current.person,
        birthDateLine: synced.person.birthDateLine,
        nationalityEthnicityReligionLine:
          synced.person.nationalityEthnicityReligionLine,
        identityDocumentLine: synced.person.identityDocumentLine,
        identityIssueLine: synced.person.identityIssueLine,
      },
      recipients: {
        ...current.recipients,
        line1: current.recipients.line1 || synced.recipients.line1,
        line2: current.recipients.line2 || synced.recipients.line2,
        archiveLine: current.recipients.archiveLine || synced.recipients.archiveLine,
      },
      suspension: {
        ...current.suspension,
        reasonLine: current.suspension.reasonLine || synced.suspension.reasonLine,
        article1Line: current.suspension.article1Line || synced.suspension.article1Line,
        article2ActionLine:
          current.suspension.article2ActionLine ||
          synced.suspension.article2ActionLine,
        executionRequestLine:
          current.suspension.executionRequestLine ||
          synced.suspension.executionRequestLine,
      },
      caseDecision: {
        ...current.caseDecision,
        prosecutionDecisionLegalBasisLine:
          current.caseDecision.prosecutionDecisionLegalBasisLine ||
          synced.caseDecision.prosecutionDecisionLegalBasisLine,
      },
      accusedDecision: {
        ...current.accusedDecision,
        prosecutionDecisionLegalBasisLine:
          current.accusedDecision.prosecutionDecisionLegalBasisLine ||
          synced.accusedDecision.prosecutionDecisionLegalBasisLine,
      },
    };
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : "Không lưu được dữ liệu biểu mẫu BM-148.");
    } finally {
      setIsSaving(false);
    }
  }
  ...
  return (
    <div className="space-y-5">
      <BmFormCasePayloadButton templateCode="BM-148" form={form} onApply={(next) => setForm(next as typeof form)} />
      ...
      <BmFormSection title="1. Cơ quan / quyết định">
      ...
      >
        <BmFieldText required label="Họ tên bị can" value={form.person.fullName} onChange={(value) => updateField("person", "fullName", value)} />
        <BmFieldText required label="Tên tội" value={form.helper.offenseName} onChange={(value) => updateField("helper", "offenseName", value)} />
        <BmFieldText label="Khoản" value={form.helper.legalClause} onChange={(value) => updateField("helper", "legalClause", value)} />
        <BmFieldText required label="Điều luật BLHS" value={form.helper.legalArticle} onChange={(value) => updateField("helper", "legalArticle", value)} />
        <BmFieldText label="Tên vụ án" value={form.helper.caseTitle} onChange={(value) => updateField("helper", "caseTitle", value)} fullWidth />
        <BmFieldText required label="Cơ quan điều tra" value={form.helper.investigationAgencyName} onChange={(value) => updateField("helper", "investigationAgencyName", value)} fullWidth />
        <BmFieldText label="Số QĐ khởi tố vụ án" value={form.helper.caseDecisionCode} onChange={(value) => updateField("helper", "caseDecisionCode", value)} />
        ...
        <BmFieldText label="Số QĐ khởi tố bị can" value={form.helper.accusedDecisionCode} onChange={(value) => updateField("helper", "accusedDecisionCode", value)} />
      ...
      <BmFormSection title="3. Căn cứ tố tụng">
        <BmFieldTextarea required label="Căn cứ BLTTHS" value={form.legalBasis.procedureArticlesLine} onChange={(value) => updateField("legalBasis", "procedureArticlesLine", value)} fullWidth />
        <BmFieldTextarea required label="Căn cứ Luật Tư pháp người chưa thành niên" value={form.legalBasis.juvenileJusticeLine} onChange={(value) => updateField("legalBasis", "juvenileJusticeLine", value)} fullWidth />
        <BmFieldTextarea required label="Căn cứ khởi tố vụ án" value={form.caseDecision.prosecutionDecisionLegalBasisLine} onChange={(value) => updateField("caseDecision", "prosecutionDecisionLegalBasisLine", value)} fullWidth />
        <BmFieldTextarea required label="Căn cứ khởi tố bị can" value={form.accusedDecision.prosecutionDecisionLegalBasisLine} onChange={(value) => updateField("accusedDecision", "prosecutionDecisionLegalBasisLine", value)} fullWidth />
      </BmFormSection>

      <BmFormSection title="4. Nội dung tạm đình chỉ">
        <BmFieldTextarea required label="Lý do xét thấy" value={form.suspension.reasonLine} onChange={(value) => updateField("suspension", "reasonLine", value)} fullWidth />
        <BmFieldTextarea required label="Điều 1" value={form.suspension.article1Line} onChange={(value) => updateField("suspension", "article1Line", value)} fullWidth />
        <BmFieldTextarea required label="Điều 2" value={form.suspension.article2ActionLine} onChange={(value) => updateField("suspension", "article2ActionLine", value)} fullWidth />
        <BmFieldTextarea required label="Điều 3" value={form.suspension.executionRequestLine} onChange={(value) => updateField("suspension", "executionRequestLine", value)} fullWidth />
      </BmFormSection>

      <BmFormSection title="5. Lý lịch bị can">
        <BmFieldSelect required label="Giới tính" value={form.person.genderText} onChange={(value) => updateField("person", "genderText", value)} options={GENDER_OPTIONS} />
        ...
        <BmFieldText label="Nơi sinh" value={form.person.placeOfBirth} onChange={(value) => updateField("person", "placeOfBirth", value)} />
        ...
        <BmFieldTextarea required label="Nơi thường trú" value={form.person.permanentResidence} onChange={(value) => updateField("person", "permanentResidence", value)} fullWidth />
        <BmFieldTextarea label="Nơi tạm trú" value={form.person.temporaryResidence} onChange={(value) => updateField("person", "temporaryResidence", value)} fullWidth />
        <BmFieldTextarea required label="Nơi ở hiện tại" value={form.person.currentResidence} onChange={(value) => updateField("person", "currentResidence", value)} fullWidth />
      </BmFormSection>

      <BmFormSection title="6. Nơi nhận">
        <BmFieldText required label="Nơi nhận dòng 1" value={form.recipients.line1} onChange={(value) => updateField("recipients", "line1", value)} />
        <BmFieldText required label="Nơi nhận dòng 2" value={form.recipients.line2} onChange={(value) => updateField("recipients", "line2", value)} />
        <BmFieldText label="Lưu hồ sơ" value={form.recipients.archiveLine} onChange={(value) => updateField("recipients", "archiveLine", value)} fullWidth />
      </BmFormSection>
