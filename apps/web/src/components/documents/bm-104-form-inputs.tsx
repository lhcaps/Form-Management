...
/**
 * BM-104 — QĐ phục hồi điều tra vụ án hình sự
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 104/HS.
 *
...
const EMPTY_FORM: Bm104Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "104/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm104Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm104Form): string {
...
function normalizeFormInputs(payload: RenderPayload | null): Bm104Form {
  const f = EMPTY_FORM;
  return {
    agency: {
...
function lookupValue(form: Bm104Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm104Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm104Form) {
  return {
    agency: {
...
export function Bm104FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm104Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm104Form, K extends keyof Bm104Form[S]>(
    section: S,
    key: K,
    value: Bm104Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-104 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-104 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-104"
        title="Dữ liệu biểu mẫu QĐ phục hồi điều tra vụ án hình sự"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-104"}
      ...
      <BmFormSection title="2. QĐ đình chỉ bị huỷ" description="Thông tin QĐ đình chỉ bị huỷ bỏ." requiredCount={2}>
        <BmFieldText
