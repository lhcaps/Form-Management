...
/**
 * BM-103 — QĐ hủy bỏ QĐ khởi tố bị can
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 103/HS.
 *
 * Căn cứ: Điều 41, 155 BLTTHS 2015.
 */
...
const EMPTY_FORM: Bm103Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "103/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm103Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm103Form): string {
...
function normalizeFormInputs(payload: RenderPayload | null): Bm103Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    cancelledDecision: {
      ...
      accusedName: nested(payload, "cancelledDecision.accusedName") || "",
      caseTitle: nested(payload, "cancelledDecision.caseTitle") || "",
      offenseName: nested(payload, "cancelledDecision.offenseName") || "",
    },
    recipients: {
...
function lookupValue(form: Bm103Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm103Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm103Form) {
  return {
    agency: {
    ...
    cancelledDecision: {
      ...
      cancelledDocumentDate: form.cancelledDecision.cancelledDocumentDate,
      cancelledDocumentAgency: form.cancelledDecision.cancelledDocumentAgency,
      accusedName: form.cancelledDecision.accusedName,
...
export function Bm103FormInputsPanel({
  ...
  const [form, setForm] = useState<Bm103Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm103Form, K extends keyof Bm103Form[S]>(
    section: S,
    key: K,
    value: Bm103Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-103 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-103 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-103"
        title="Dữ liệu biểu mẫu QĐ hủy bỏ QĐ khởi tố bị can"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-103"}
      ...
      <BmFormSection title="2. QĐ khởi tố bị can bị huỷ" description="Thông tin QĐ khởi tố bị can cần huỷ bỏ." requiredCount={2}>
        <BmFieldText
          label="Số QĐ khởi tố bị can"
          required
          value={form.cancelledDecision.cancelledDocumentCode}
          onChange={(v) => patch("cancelledDecision", "cancelledDocumentCode", v)}
        />
        <BmFieldDate
          label="Ngày QĐ khởi tố bị can"
          value={form.cancelledDecision.cancelledDocumentDate}
          onChange={(v) => patch("cancelledDecision", "cancelledDocumentDate", v)}
        />
        <BmFieldText
          label="Cơ quan ban hành QĐ"
        ...
        <BmFieldText
          label="Tên bị can"
          required
          value={form.cancelledDecision.accusedName}
          onChange={(v) => patch("cancelledDecision", "accusedName", v)}
        />
        <BmFieldText
          label="Tên vụ án"
