...
/**
 * BM-126 — QĐ không phê chuẩn lệnh bắt người
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 126/HS.
 *
...
const EMPTY_FORM: Bm126Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "126/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm126Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm126Form): string {
...
function normalizeFormInputs(payload: RenderPayload | null): Bm126Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    decision: {
      ...
      accusedName: nested(payload, "decision.accusedName") || "",
      caseTitle: nested(payload, "decision.caseTitle") || "",
      offenseName: nested(payload, "decision.offenseName") || "",
      warrantCode: nested(payload, "decision.warrantCode") || "",
      warrantDate: nested(payload, "decision.warrantDate") || "",
      warrantAgency: nested(payload, "decision.warrantAgency") || "",
      refusalReason: nested(payload, "decision.refusalReason") || "",
    },
    recipients: {
...
function lookupValue(form: Bm126Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm126Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm126Form) {
  return {
    agency: {
    ...
    decision: {
      ...
      accusedName: form.decision.accusedName,
      caseTitle: form.decision.caseTitle,
      offenseName: form.decision.offenseName,
      warrantCode: form.decision.warrantCode,
      warrantDate: form.decision.warrantDate,
      warrantAgency: form.decision.warrantAgency,
      refusalReason: form.decision.refusalReason,
...
export function Bm126FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm126Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm126Form, K extends keyof Bm126Form[S]>(
    section: S,
    key: K,
    value: Bm126Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-126 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-126 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-126"
        title="Dữ liệu biểu mẫu QĐ không phê chuẩn lệnh bắt người"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-126"}
      ...
      <BmFormSection title="2. Thông tin lệnh bắt bị từ chối" description="Nhập thông tin lệnh bắt không được phê chuẩn." requiredCount={2}>
        <BmFieldTextarea
        ...
        />
        <BmFieldText
          label="Tên người bị bắt"
          required
          value={form.decision.accusedName}
          onChange={(v) => patch("decision", "accusedName", v)}
        />
        <BmFieldText
          label="Tên vụ án"
          required
        ...
        <BmFieldText
          label="Số lệnh bắt"
          required
          value={form.decision.warrantCode}
          onChange={(v) => patch("decision", "warrantCode", v)}
        />
        <BmFieldDate
          label="Ngày lệnh bắt"
          value={form.decision.warrantDate}
          onChange={(v) => patch("decision", "warrantDate", v)}
        />
        <BmFieldText
          label="Cơ quan ban hành lệnh"
          value={form.decision.warrantAgency}
          onChange={(v) => patch("decision", "warrantAgency", v)}
        />
        <BmFieldTextarea
          label="Lý do không phê chuẩn"
          required
          fullWidth
          value={form.decision.refusalReason}
          onChange={(v) => patch("decision", "refusalReason", v)}
          rows={3}
        />
        <BmFieldTextarea
          label="Lý do tự sinh"
