...
/**
 * BM-127 — Yêu cầu cung cấp hồ sơ, tài liệu
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 127/HS.
 *
 * Căn cứ: Điều 41, 89 BLTTHS 2015.
 */
...
const EMPTY_FORM: Bm127Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "127/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm127Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}



function normalizeFormInputs(payload: RenderPayload | null): Bm127Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    request: {
      requestedAgency: nested(payload, "request.requestedAgency") || "",
      requestCode: nested(payload, "request.requestCode") || "",
      requestDate: nested(payload, "request.requestDate") || "",
      requestContent: nested(payload, "request.requestContent") || "",
      deadlineDays: nested(payload, "request.deadlineDays") || "",
    },
    recipients: {
...
function lookupValue(form: Bm127Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm127Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm127Form) {
  return {
    agency: {
    ...
    request: {
      requestedAgency: form.request.requestedAgency,
      requestCode: form.request.requestCode,
      requestDate: form.request.requestDate,
      requestContent: form.request.requestContent,
      deadlineDays: form.request.deadlineDays,
...
export function Bm127FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm127Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm127Form, K extends keyof Bm127Form[S]>(
    section: S,
    key: K,
    value: Bm127Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-127 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-127 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-127"
        title="Dữ liệu biểu mẫu Yêu cầu cung cấp hồ sơ, tài liệu"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-127"}
      ...
      <BmFormSection title="2. Yêu cầu cung cấp" description="Thông tin yêu cầu cung cấp hồ sơ tài liệu." requiredCount={2}>
        <BmFieldText
          label="Cơ quan được yêu cầu"
          required
          value={form.request.requestedAgency}
          onChange={(v) => patch("request", "requestedAgency", v)}
        />
        <BmFieldText
          label="Số yêu cầu"
          value={form.request.requestCode}
          onChange={(v) => patch("request", "requestCode", v)}
        />
        <BmFieldDate
          label="Ngày yêu cầu"
          value={form.request.requestDate}
          onChange={(v) => patch("request", "requestDate", v)}
        />
        <BmFieldTextarea
          label="Nội dung yêu cầu"
          required
          fullWidth
          value={form.request.requestContent}
          onChange={(v) => patch("request", "requestContent", v)}
          rows={3}
        />
        <BmFieldText
          label="Thời hạn (ngày)"
          value={form.request.deadlineDays}
          onChange={(v) => patch("request", "deadlineDays", v)}
        />
