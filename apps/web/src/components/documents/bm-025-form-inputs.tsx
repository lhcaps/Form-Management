...
const EMPTY_FORM: BM025FormInputs = {
  agency: {
...
function normalizeForm(source: unknown): BM025FormInputs {
...
function derive(form: BM025FormInputs): BM025FormInputs {
  return {
    ...form,
    document: {
      ...form.document,
...
export function Bm025FormInputsPanel({ documentId, onSaved }: Props) {
  const [form, setForm] = useState<BM025FormInputs>(EMPTY_FORM);
  ...
  function updateSection(sectionName: keyof BM025FormInputs, fieldName: string, value: string) {
    setForm((current) => {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setSavedAt(new Date());
      setSuccessMessage(`Đã lưu dữ liệu BM-025. Có thể render lại DOCX/PDF.`);
      onSaved?.();
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : `Lưu BM-025 thất bại`);
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function handleReload() {
    ...
    try {
      ...
      setInitialSnapshot(JSON.stringify(derive(next)));
      setSavedAt(null);
      setSuccessMessage(`Đã tải lại dữ liệu BM-025.`);
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : `Tải lại BM-025 thất bại`);
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-4">
      <BmFormMetaBar
        ...
        warningMessage={null}
        primaryLabel={`Lưu dữ liệu BM-025`}
        ...
        extraActions={
          <BmFormCasePayloadButton
            templateCode="BM-025"
            form={form}
            onApply={(next) => setForm(next as BM025FormInputs)}
          />
        }
      />
      ...
      <BmFormSection title="5. Nội dung quyết định">
        ...
        />
        <BmFieldTextarea
          label={`Nội dung quyết định (${"bổ sung Quyết định khởi tố vụ án hình sự"})`}
