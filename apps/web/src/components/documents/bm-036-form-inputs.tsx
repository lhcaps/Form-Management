...
/**
 * BM-036 — Quyết định trả tự do cho người bị tạm giữ
...
const EMPTY_FORM: BM036FormInputs = {
  agency: {
  ...
  legalBasis: {
    procedureArticlesLine: "Căn cứ các điều 36, 41, 110 và 120 của Bộ luật Tố tụng hình sự;",
  },
  decision: {
    summaryLine: "",
    decisionLine: "",
...
function normalizeForm(source: unknown): BM036FormInputs {
...
function derive(form: BM036FormInputs): BM036FormInputs {
  return {
    ...form,
    document: {
      ...form.document,
...
export function Bm036FormInputsPanel({ documentId, onSaved }: Props) {
  const [form, setForm] = useState<BM036FormInputs>(EMPTY_FORM);
  ...
  function updateSection(sectionName: keyof BM036FormInputs, fieldName: string, value: string) {
    setForm((current) => {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setSavedAt(new Date());
      setSuccessMessage(`Đã lưu dữ liệu BM-036. Có thể render lại DOCX/PDF.`);
      onSaved?.();
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : `Lưu BM-036 thất bại`);
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function handleReload() {
    ...
    try {
      ...
      setInitialSnapshot(JSON.stringify(derive(next)));
      setSavedAt(null);
      setSuccessMessage(`Đã tải lại dữ liệu BM-036.`);
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : `Tải lại BM-036 thất bại`);
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-4">
      <BmFormMetaBar
        ...
        warningMessage={null}
        primaryLabel={`Lưu dữ liệu BM-036`}
        ...
        extraActions={
          <BmFormCasePayloadButton
            templateCode="BM-036"
            form={form}
            onApply={(next) => setForm(next as BM036FormInputs)}
          />
        }
      />
      ...
      <BmFormSection title="5. Nội dung quyết định">
        ...
        />
        <BmFieldTextarea
          label={`Nội dung quyết định (${"trả tự do cho người bị tạm giữ"})`}
