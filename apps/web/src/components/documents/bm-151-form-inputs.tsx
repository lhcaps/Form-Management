...
/**
 * BM-151 — Quyết định huỷ bỏ QĐ đình chỉ vụ án
 * Stage: TRUY_TO, Group: G05. TT 03/2026-VKSTC, Mẫu số 151/HS.
 *
...
function buildIssuePlaceAndDateLine(form: Bm151Form): string {
  return issuePlaceDateLine(form.agency.issuePlace, form.document.issueDateIso);
}

function normalizeFormInputs(payload: RenderPayload | null): Bm151Form {
  const f = EMPTY_FORM;
  if (!payload) return f;
  return {
    agency: {
...
function lookupValue(form: Bm151Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
...
function validateForm(form: Bm151Form): string[] {
  return REQUIRED_FIELDS.filter(([, path]) => !lookupValue(form, path)).map(
    ([label]) => label,
  );
}

function buildSaveBody(form: Bm151Form) {
  return {
    agency: {
...
export function Bm151FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm151Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm151Form, K extends keyof Bm151Form[S]>(
    section: S,
    key: K,
    value: Bm151Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-151 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-151 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-151"
        title="Dữ liệu biểu mẫu Quyết định huỷ bỏ QĐ đình chỉ vụ án"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-151"}
