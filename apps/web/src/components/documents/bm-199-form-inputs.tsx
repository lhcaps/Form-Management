...
/**
 * BM-199 — Kiến nghị về quyết định áp dụng BPXLCH của Tòa án
 * Stage: NGUOI_CHUA_THANH_NIEN (Thủ tục đối với người chưa thành niên), Group: G04
 *
...
export function Bm199FormInputsPanel({ documentId, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setIsLoading(true);
        if (isMounted) setPayload(await getDocumentRenderPayload<Record<string, unknown>>(documentId));
      } catch (e) {
  ...
  return (
    <div className="space-y-4">
      <BmFormMetaBar
        title="Kiến nghị về quyết định áp dụng BPXLCH của Tòa án"
        subtitle={`Biểu mẫu TT 03/2026-VKSTC · Stage: NGUOI_CHUA_THANH_NIEN (Thủ tục đối với người chưa thành niên) · Group: G04 · Scope: ${scope}`}
        templateCode="BM-199"
        isDirty={isDirty}
        isLoading={isLoading}
        errorMessage={error}
        ...
        meta={
          <div className="text-xs text-slate-500">
            <div>Stage: <span className="font-mono">NGUOI_CHUA_THANH_NIEN</span></div>
