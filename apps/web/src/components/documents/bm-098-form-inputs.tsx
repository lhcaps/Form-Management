...
/**
 * BM-098 — Yêu cầu ra QĐ thay đổi QĐ khởi tố bị can
 * Stage: KHOI_TO, Group: G02. TT 03/2026-VKSTC, Mẫu số 98/HS.
 *
 * Căn cứ: Điều 36, 37, 167 BLTTHS.
 * Nghiệp vụ: VKS yêu cầu CQĐT ra QĐ thay đổi QĐ khởi tố bị can.
 */
...
const EMPTY_FORM: Bm098Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "98/YCT-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
const REQUIRED_FIELDS: ReadonlyArray<[keyof Bm098Form, string, string]> = [
  ...
  ["request", "changeReason", "Lý do thay đổi"],
...
function buildIssuePlaceAndDateLine(form: Bm098Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm098Form): string {
  const r = form.request;
  if (!r.accusedFullName || !r.caseTitle) return "";
  return `Qua xét nội dung vụ án ${r.caseTitle.trim()}, có căn cứ cho rằng QĐ số ${r.originalDecisionCode.trim()}${r.originalDecisionDate ? ` ngày ${cleanText(r.originalDecisionDate)}` : ""} về việc khởi tố bị can ${r.accusedFullName.trim()} về tội "${r.offenseName.trim()}" cần được thay đổi vì ${r.changeReason.trim()}.`;
}

function normalizeFormInputs(payload: RenderPayload | null): Bm098Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    request: {
      ...
      changeReason: nested(payload, "request.changeReason") || "",
    },
    recipients: {
...
function validateForm(form: Bm098Form): string[] {
...
function buildSaveBody(form: Bm098Form) {
  ...
  return {
    ...
    request: {
      ...
      changeReason: form.request.changeReason,
...
export function Bm098FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm098Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm098Form, K extends keyof Bm098Form[S]>(
    section: S,
    key: K,
    value: Bm098Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-098 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-098 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-098"
        title="Dữ liệu biểu mẫu Yêu cầu ra QĐ thay đổi QĐ khởi tố bị can"
        subtitle="Biểu mẫu TT 03/2026-VKSTC · Mẫu số 98/HS · Stage KHOI_TO (G02). Căn cứ Điều 36, 37, 167 BLTTHS."
      ...
      <BmFormSection
        ...
        description="Nhập thông tin QĐ khởi tố bị can cần thay đổi và lý do thay đổi."
      >
        <BmFieldTextarea
        ...
        <BmFieldTextarea
          ...
          required
          value={form.request.changeReason}
          onChange={(v) => patch("request", "changeReason", v)}
          rows={2}
        />
        <BmFieldTextarea
