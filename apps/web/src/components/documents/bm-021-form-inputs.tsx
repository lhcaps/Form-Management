...
const EMPTY_FORM: BM021FormInputs = {
  agency: {
  ...
  legalBasis: {
    procedureArticlesLine: "Căn cứ các điều 36, 39 và 148 của Bộ luật Tố tụng hình sự;",
  },
  decision: {
    summaryLine: "",
    decisionLine: "",
...
function normalizeForm(source: unknown): BM021FormInputs {
...
function derive(form: BM021FormInputs): BM021FormInputs {
  return {
    ...form,
    document: {
      ...form.document,
...
export function Bm021FormInputsPanel({ documentId, onSaved }: Props) {
  const [form, setForm] = useState<BM021FormInputs>(EMPTY_FORM);
  ...
  function updateSection(sectionName: keyof BM021FormInputs, fieldName: string, value: string) {
    setForm((current) => {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setSavedAt(new Date());
      setSuccessMessage(`Đã lưu dữ liệu BM-021. Có thể render lại DOCX/PDF.`);
      onSaved?.();
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : `Lưu BM-021 thất bại`);
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function handleReload() {
    ...
    try {
      ...
      setInitialSnapshot(JSON.stringify(derive(next)));
      setSavedAt(null);
      setSuccessMessage(`Đã tải lại dữ liệu BM-021.`);
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : `Tải lại BM-021 thất bại`);
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-4">
      <BmFormMetaBar
        ...
        warningMessage={null}
        primaryLabel={`Lưu dữ liệu BM-021`}
        ...
        extraActions={
          <BmFormCasePayloadButton
            templateCode="BM-021"
            form={form}
            onApply={(next) => setForm(next as BM021FormInputs)}
          />
        }
      />
      ...
      <BmFormSection title="5. Nội dung quyết định">
        ...
        />
        <BmFieldTextarea
          label={`Nội dung quyết định (${"không khởi tố vụ án hình sự"})`}
