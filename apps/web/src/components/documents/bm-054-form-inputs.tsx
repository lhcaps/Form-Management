...
type SectionKey = keyof Bm054FormInputs;
...
type Bm054FormInputsPanelProps = {
  documentId: string | number;
  onSaved?: () => void;
};

const EMPTY_FORM: Bm054FormInputs = {
  agency: {
    parentName: "",
    name: "",
    shortName: "",
  ...
  notification: {
    title: "THÔNG BÁO",
    subject: "Về việc áp dụng biện pháp cấm đi khỏi nơi cư trú",
    content: "",
...
const REQUIRED_FIELDS: RequiredField[] = [
  ...
  { section: "document", field: "issueDate", label: "Ngày thông báo" },
...
function getValue(form: Bm054FormInputs, sectionKey: SectionKey, field: string): string {
...
function normalizeFormInputs(payload: Record<string, unknown>): Bm054FormInputs {
  ...
  return {
    ...
    notification: {
      title: pick(notification, "title") || "THÔNG BÁO",
      subject:
...
async function getBm054RenderPayload(documentId: string | number): Promise<Record<string, unknown>> {
  return await getDocumentRenderPayload<Record<string, unknown>>(documentId);
}

async function saveBm054FormInputs(
  documentId: string | number,
  form: Bm054FormInputs,
): Promise<void> {
...
export function Bm054FormInputsPanel({
  documentId,
  onSaved,
}: Bm054FormInputsPanelProps) {
  const [form, setForm] = useState<Bm054FormInputs>(EMPTY_FORM);
  ...
  async function loadForm() {
    ...
    try {
      const payload = await getBm054RenderPayload(documentId);
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "Không tải được dữ liệu biểu mẫu BM-054.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  function fillCustomerSample() {
    const sample: Bm054FormInputs = {
      ...
      notification: {
        ...
        preventiveMeasureOrderCode: "12/LCCT-VKS",
        preventiveMeasureOrderIssueDate: "2026-03-04",
      },
      recipients: {
        monitoringUnitLine:
          "- Ủy ban nhân dân xã Đông Thạnh, Thành phố Hồ Chí Minh;",
  ...
  async function handleSave() {
    ...
    try {
      await saveBm054FormInputs(documentId, form);
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "Không lưu được dữ liệu biểu mẫu BM-054.",
      );
    } finally {
      setIsSaving(false);
    }
  ...
  if (isLoading) {
    return (
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <p className="text-sm text-slate-600">Đang tải dữ liệu BM-054...</p>
      </section>
    );
  }
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <h2 className="mt-1 text-xl font-bold text-slate-950">
              Dữ liệu biểu mẫu Thông báo áp dụng biện pháp cấm đi khỏi nơi cư trú
            </h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
              Form này lưu dữ liệu nghiệp vụ riêng cho BM-054: số thông báo,
              số lệnh cấm, ngày lệnh cấm, thông tin bị can, biện pháp, đơn vị
              quản lý, nơi nhận và chữ ký.
            </p>
          </div>
        ...
        {missingFields.length > 0 ? (
          ...
          <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700">
            Các trường quan trọng của BM-054 đã được nhập đủ.
          </div>
        )}
      ...
      <FormActionBar printHidden={false}>
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <p className="text-sm text-slate-600">
            ...
            <b> issuePlaceAndDateLine</b> và <b>notification.preventiveMeasureOrderLine</b>.
          </p>
          ...
          >
            {isSaving ? "Đang lưu..." : "Lưu dữ liệu BM-054"}
          </button>
        </div>
      </FormActionBar>
    </div>
