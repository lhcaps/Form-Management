 ...
 * Stage: PHUC_HOI_XAC_MINH, Group: G99. TT 03/2026-VKSTC, Mẫu số 133/HS.
 *
 * Căn cứ: Điều 36, 37, 51 BLTTHS 2015.
 * Nghiệp vụ: VKS ra QĐ giám định lại trong trường hợp đặc biệt.
 */
...
const EMPTY_FORM: Bm133Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "133/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm133Form): string {
...
function buildReasonLine(form: Bm133Form): string {
  ...
  const parts: string[] = [
    ...
    ` của ${d.expertName ? `giám định viên ${d.expertName.trim()}` : d.originalAppraisalAgency.trim()}.`,
    ` Trong trường hợp đặc biệt, cần giám định lại vì ${d.specialReason.trim()}.`,
  ];
  return parts.filter(Boolean).join("");
}

function normalizeFormInputs(payload: RenderPayload | null): Bm133Form {
  const f = EMPTY_FORM;
  if (!payload) return f;
  return {
    agency: {
    ...
    decision: {
      ...
      expertName: nested(payload, "decision.expertName") || "",
...
function lookupValue(form: Bm133Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
...
function validateForm(form: Bm133Form): string[] {
  return REQUIRED_FIELDS.filter(([, path]) => !lookupValue(form, path)).map(([label]) => label);
}

function buildSaveBody(form: Bm133Form) {
  return {
    agency: {
    ...
    decision: {
      ...
      expertName: form.decision.expertName,
...
export function Bm133FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm133Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm133Form, K extends keyof Bm133Form[S]>(
    section: S,
    key: K,
    value: Bm133Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-133 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-133 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-133"
        title="Dữ liệu biểu mẫu QĐ giám định lại trong trường hợp đặc biệt"
        subtitle="Biểu mẫu TT 03/2026-VKSTC · Mẫu số 133/HS · Căn cứ Điều 36, 37, 51 BLTTHS 2015."
        isDirty={false}
        isLoading={loading}
        isSaving={saving}
        savedAt={null}
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-133"}
      ...
      <BmFormSection
        title="2. Thông tin giám định lại đặc biệt"
        description="Nhập thông tin giám định lại trong trường hợp đặc biệt."
      >
        <BmFieldTextarea
        ...
        />
        <BmFieldText
          label="Tên giám định viên"
          value={form.decision.expertName}
          onChange={(v) => patch("decision", "expertName", v)}
        />
        <BmFieldText
        ...
        />
        <BmFieldTextarea
          label="Lý do đặc biệt cần giám định lại"
          required
          fullWidth
