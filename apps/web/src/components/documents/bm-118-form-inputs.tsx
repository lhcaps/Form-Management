...
/**
 * BM-118 — QĐ phục hồi việc giám định
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 118/HS.
 *
...
const EMPTY_FORM: Bm118Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "118/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm118Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}



function normalizeFormInputs(payload: RenderPayload | null): Bm118Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    decision: {
      ...
      resumptionReason: nested(payload, "decision.resumptionReason") || "",
    },
    recipients: {
...
function lookupValue(form: Bm118Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm118Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm118Form) {
  return {
    agency: {
    ...
    decision: {
      ...
      resumptionReason: form.decision.resumptionReason,
...
export function Bm118FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm118Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm118Form, K extends keyof Bm118Form[S]>(
    section: S,
    key: K,
    value: Bm118Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-118 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-118 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-118"
        title="Dữ liệu biểu mẫu QĐ phục hồi việc giám định"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-118"}
      ...
      <BmFormSection title="2. Thông tin phục hồi giám định" description="Nhập thông tin QĐ phục hồi giám định." requiredCount={2}>
        <BmFieldText
          label="Tên vụ án"
          required
        ...
        <BmFieldTextarea
          ...
          required
          fullWidth
          value={form.decision.resumptionReason}
          onChange={(v) => patch("decision", "resumptionReason", v)}
          rows={3}
        />
