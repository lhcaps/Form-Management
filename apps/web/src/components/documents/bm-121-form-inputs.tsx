...
/**
 * BM-121 — QĐ thay đổi biện pháp ngăn chặn
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 121/HS.
 *
...
type Bm121Form = {
  ...
  measureChange: MeasureChangeForm;
...
const EMPTY_FORM: Bm121Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "121/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm121Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}



function normalizeFormInputs(payload: RenderPayload | null): Bm121Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    measureChange: {
      accusedName: nested(payload, "measureChange.accusedName") || "",
      caseTitle: nested(payload, "measureChange.caseTitle") || "",
      offenseName: nested(payload, "measureChange.offenseName") || "",
      oldMeasure: nested(payload, "measureChange.oldMeasure") || "",
      newMeasure: nested(payload, "measureChange.newMeasure") || "",
      reasonLine: nested(payload, "measureChange.reasonLine") || "",
    },
    recipients: {
...
function lookupValue(form: Bm121Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm121Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm121Form) {
  return {
    agency: {
    ...
    measureChange: {
      accusedName: form.measureChange.accusedName,
      caseTitle: form.measureChange.caseTitle,
      offenseName: form.measureChange.offenseName,
      oldMeasure: form.measureChange.oldMeasure,
      newMeasure: form.measureChange.newMeasure,
...
export function Bm121FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm121Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm121Form, K extends keyof Bm121Form[S]>(
    section: S,
    key: K,
    value: Bm121Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-121 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-121 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-121"
        title="Dữ liệu biểu mẫu QĐ thay đổi biện pháp ngăn chặn"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-121"}
      ...
      <BmFormSection title="2. Thay đổi biện pháp ngăn chặn" description="Thông tin biện pháp ngăn chặn được thay đổi." requiredCount={2}>
        <BmFieldText
          label="Tên bị can/bị cáo"
          required
          value={form.measureChange.accusedName}
          onChange={(v) => patch("measureChange", "accusedName", v)}
        />
        <BmFieldText
          label="Tên vụ án"
          required
          value={form.measureChange.caseTitle}
          onChange={(v) => patch("measureChange", "caseTitle", v)}
        />
        <BmFieldText
          label="Tội danh"
          value={form.measureChange.offenseName}
          onChange={(v) => patch("measureChange", "offenseName", v)}
        />
        <BmFieldText
          label="Biện pháp cũ"
          value={form.measureChange.oldMeasure}
          onChange={(v) => patch("measureChange", "oldMeasure", v)}
        />
        <BmFieldText
          label="Biện pháp mới"
          required
          value={form.measureChange.newMeasure}
          onChange={(v) => patch("measureChange", "newMeasure", v)}
        />
        <BmFieldTextarea
          label="Lý do thay đổi"
          required
          fullWidth
          value={form.measureChange.reasonLine}
          onChange={(v) => patch("measureChange", "reasonLine", v)}
          rows={3}
        />
