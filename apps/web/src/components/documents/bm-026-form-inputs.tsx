...
/**
 * BM-026 — Quyết định huỷ bỏ Quyết định khởi tố vụ án hình sự
...
const EMPTY_FORM: BM026FormInputs = {
  agency: {
  ...
  legalBasis: {
    procedureArticlesLine: "Căn cứ các điều 36, 41 và 157 của Bộ luật Tố tụng hình sự;",
  },
  decision: {
    summaryLine: "",
    decisionLine: "",
...
function normalizeForm(source: unknown): BM026FormInputs {
...
function derive(form: BM026FormInputs): BM026FormInputs {
  return {
    ...form,
    document: {
      ...form.document,
...
export function Bm026FormInputsPanel({ documentId, onSaved }: Props) {
  const [form, setForm] = useState<BM026FormInputs>(EMPTY_FORM);
  ...
  function updateSection(sectionName: keyof BM026FormInputs, fieldName: string, value: string) {
    setForm((current) => {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setSavedAt(new Date());
      setSuccessMessage(`Đã lưu dữ liệu BM-026. Có thể render lại DOCX/PDF.`);
      onSaved?.();
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : `Lưu BM-026 thất bại`);
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function handleReload() {
    ...
    try {
      ...
      setInitialSnapshot(JSON.stringify(derive(next)));
      setSavedAt(null);
      setSuccessMessage(`Đã tải lại dữ liệu BM-026.`);
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : `Tải lại BM-026 thất bại`);
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-4">
      <BmFormMetaBar
        ...
        warningMessage={null}
        primaryLabel={`Lưu dữ liệu BM-026`}
        ...
        extraActions={
          <BmFormCasePayloadButton
            templateCode="BM-026"
            form={form}
            onApply={(next) => setForm(next as BM026FormInputs)}
          />
        }
      />
      ...
      <BmFormSection title="5. Nội dung quyết định">
        ...
        />
        <BmFieldTextarea
          label={`Nội dung quyết định (${"huỷ bỏ Quyết định khởi tố vụ án hình sự"})`}
