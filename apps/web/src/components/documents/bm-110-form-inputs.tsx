 ...
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 110/HS.
 *
...
const EMPTY_FORM: Bm110Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "110/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm110Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm110Form): string {
...
function normalizeFormInputs(payload: RenderPayload | null): Bm110Form {
  const f = EMPTY_FORM;
  return {
    agency: {
...
function lookupValue(form: Bm110Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm110Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm110Form) {
  return {
    agency: {
...
export function Bm110FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm110Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm110Form, K extends keyof Bm110Form[S]>(
    section: S,
    key: K,
    value: Bm110Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-110 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-110 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-110"
        title="Dữ liệu biểu mẫu QĐ huỷ bỏ QĐ đình chỉ điều tra VAHS"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-110"}
      ...
      <BmFormSection title="2. Quyết định bị huỷ bỏ" description="Thông tin QĐ đình chỉ điều tra VAHS cần huỷ bỏ." requiredCount={2}>
        <BmFieldText
          label="Số QĐ bị huỷ"
          required
