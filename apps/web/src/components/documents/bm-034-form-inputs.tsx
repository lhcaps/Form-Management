...
/**
 * BM-034 — Quyết định không phê chuẩn Quyết định gia hạn tạm giữ
...
const EMPTY_FORM: BM034FormInputs = {
  agency: {
  ...
  legalBasis: {
    procedureArticlesLine: "Căn cứ các điều 36, 41, 110 và 118 của Bộ luật Tố tụng hình sự;",
  },
  decision: {
    summaryLine: "",
    decisionLine: "",
...
function normalizeForm(source: unknown): BM034FormInputs {
...
function derive(form: BM034FormInputs): BM034FormInputs {
  return {
    ...form,
    document: {
      ...form.document,
...
export function Bm034FormInputsPanel({ documentId, onSaved }: Props) {
  const [form, setForm] = useState<BM034FormInputs>(EMPTY_FORM);
  ...
  function updateSection(sectionName: keyof BM034FormInputs, fieldName: string, value: string) {
    setForm((current) => {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setSavedAt(new Date());
      setSuccessMessage(`Đã lưu dữ liệu BM-034. Có thể render lại DOCX/PDF.`);
      onSaved?.();
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : `Lưu BM-034 thất bại`);
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function handleReload() {
    ...
    try {
      ...
      setInitialSnapshot(JSON.stringify(derive(next)));
      setSavedAt(null);
      setSuccessMessage(`Đã tải lại dữ liệu BM-034.`);
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : `Tải lại BM-034 thất bại`);
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-4">
      <BmFormMetaBar
        ...
        warningMessage={null}
        primaryLabel={`Lưu dữ liệu BM-034`}
        ...
        extraActions={
          <BmFormCasePayloadButton
            templateCode="BM-034"
            form={form}
            onApply={(next) => setForm(next as BM034FormInputs)}
          />
        }
      />
      ...
      <BmFormSection title="5. Nội dung quyết định">
        ...
        />
        <BmFieldTextarea
          label={`Nội dung quyết định (${"không phê chuẩn Quyết định gia hạn tạm giữ"})`}
