...
/**
 * BM-041 — Quyết định không phê chuẩn Lệnh tạm giam
...
const EMPTY_FORM: BM041FormInputs = {
  agency: {
  ...
  legalBasis: {
    procedureArticlesLine: "Căn cứ các điều 36, 41, 110 và 122 của Bộ luật Tố tụng hình sự;",
  },
  decision: {
    summaryLine: "",
    decisionLine: "",
...
function normalizeForm(source: unknown): BM041FormInputs {
...
function derive(form: BM041FormInputs): BM041FormInputs {
  return {
    ...form,
    document: {
      ...form.document,
...
export function Bm041FormInputsPanel({ documentId, onSaved }: Props) {
  const [form, setForm] = useState<BM041FormInputs>(EMPTY_FORM);
  ...
  function updateSection(sectionName: keyof BM041FormInputs, fieldName: string, value: string) {
    setForm((current) => {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setSavedAt(new Date());
      setSuccessMessage(`Đã lưu dữ liệu BM-041. Có thể render lại DOCX/PDF.`);
      onSaved?.();
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : `Lưu BM-041 thất bại`);
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function handleReload() {
    ...
    try {
      ...
      setInitialSnapshot(JSON.stringify(derive(next)));
      setSavedAt(null);
      setSuccessMessage(`Đã tải lại dữ liệu BM-041.`);
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : `Tải lại BM-041 thất bại`);
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-4">
      <BmFormMetaBar
        ...
        warningMessage={null}
        primaryLabel={`Lưu dữ liệu BM-041`}
        ...
        extraActions={
          <BmFormCasePayloadButton
            templateCode="BM-041"
            form={form}
            onApply={(next) => setForm(next as BM041FormInputs)}
          />
        }
      />
      ...
      <BmFormSection title="5. Nội dung quyết định">
        ...
        />
        <BmFieldTextarea
          label={`Nội dung quyết định (${"không phê chuẩn Lệnh tạm giam"})`}
