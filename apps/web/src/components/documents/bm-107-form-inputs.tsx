...
/**
 * BM-107 — QĐ không chấp nhận yêu cầu gia hạn điều tra VAHS
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 107/HS.
 *
...
const EMPTY_FORM: Bm107Form = {
  ...
  document: { documentCode: "107/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function lookupValue(form: Bm107Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
export function Bm107FormInputsPanel({
  ...
  const patch = <S extends keyof Bm107Form, K extends keyof Bm107Form[S]>(
    section: S,
    key: K,
    value: Bm107Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-107 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-107 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-107"
        title="Dữ liệu biểu mẫu QĐ không chấp nhận yêu cầu gia hạn điều tra VAHS"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-107"}
      ...
      <BmFormSection title="2. Yêu cầu gia hạn bị từ chối" description="Thông tin yêu cầu gia hạn bị từ chối." requiredCount={2}>
        <BmFieldText
          label="Tên vụ án"
          required
