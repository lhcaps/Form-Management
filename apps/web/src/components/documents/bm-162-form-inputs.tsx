...
/**
 * BM-162 — Giấy mời
 * Stage: TRUY_TO (Truy tố, chuyển hồ sơ), Group: G03
 *
...
export function Bm162FormInputsPanel({ documentId, onSaved }: Props) {
  ...
  return (
    <div className="space-y-4">
      <BmFormMetaBar
        title="Giấy mời"
        subtitle={`Biểu mẫu TT 03/2026-VKSTC · Stage: TRUY_TO (Truy tố, chuyển hồ sơ) · Group: G03 · Scope: ${scope}`}
        templateCode="BM-162"
        isDirty={isDirty}
        isLoading={isLoading}
        errorMessage={error}
        ...
        meta={
          <div className="text-xs text-slate-500">
            <div>Stage: <span className="font-mono">TRUY_TO</span></div>
            <div>Group: <span className="font-mono">G03</span></div>
          </div>
        }
      />
