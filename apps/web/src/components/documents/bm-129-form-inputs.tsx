 ...
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 129/HS.
 *
 * Căn cứ: Điều 41, 51 BLTTHS 2015.
 */
...
const EMPTY_FORM: Bm129Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "129/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm129Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm129Form): string {
...
function normalizeFormInputs(payload: RenderPayload | null): Bm129Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    decision: {
      ...
      supplementReason: nested(payload, "decision.supplementReason") || "",
    },
    recipients: {
...
function lookupValue(form: Bm129Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm129Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm129Form) {
  return {
    agency: {
    ...
    decision: {
      ...
      supplementReason: form.decision.supplementReason,
...
export function Bm129FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm129Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm129Form, K extends keyof Bm129Form[S]>(
    section: S,
    key: K,
    value: Bm129Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-129 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-129 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-129"
        title="Dữ liệu biểu mẫu QĐ trưng cầu giám định bổ sung"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-129"}
      ...
      <BmFormSection title="2. Thông tin trưng cầu giám định bổ sung" description="Nhập thông tin giám định bổ sung." requiredCount={2}>
        <BmFieldTextarea
        ...
        />
        <BmFieldTextarea
          label="Lý do giám định bổ sung"
          required
          fullWidth
          value={form.decision.supplementReason}
          onChange={(v) => patch("decision", "supplementReason", v)}
          rows={3}
        />
        <BmFieldTextarea
          label="Lý do tự sinh"
