...
/**
 * BM-136 — QĐ áp dụng biện pháp đặt tiền để bảo đảm
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 136/HS.
 *
 * Căn cứ: Điều 41, 121 BLTTHS 2015.
 */
...
const EMPTY_FORM: Bm136Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "136/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm136Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}



function normalizeFormInputs(payload: RenderPayload | null): Bm136Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    moneyDeposit: {
      accusedName: nested(payload, "moneyDeposit.accusedName") || "",
      caseTitle: nested(payload, "moneyDeposit.caseTitle") || "",
      offenseName: nested(payload, "moneyDeposit.offenseName") || "",
      bondAmount: nested(payload, "moneyDeposit.bondAmount") || "",
      reasonLine: nested(payload, "moneyDeposit.reasonLine") || "",
    },
    recipients: {
...
function lookupValue(form: Bm136Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function buildSaveBody(form: Bm136Form) {
  return {
    agency: {
    ...
    moneyDeposit: {
      accusedName: form.moneyDeposit.accusedName,
      caseTitle: form.moneyDeposit.caseTitle,
      offenseName: form.moneyDeposit.offenseName,
      bondAmount: form.moneyDeposit.bondAmount,
...
export function Bm136FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm136Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm136Form, K extends keyof Bm136Form[S]>(
    section: S,
    key: K,
    value: Bm136Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-136 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-136 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-136"
        title="Dữ liệu biểu mẫu QĐ áp dụng biện pháp đặt tiền để bảo đảm"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-136"}
      ...
      <BmFormSection title="2. Áp dụng biện pháp đặt tiền" description="Thông tin biện pháp đặt tiền bảo đảm." requiredCount={2}>
        <BmFieldText
          label="Tên bị can/bị cáo"
          required
          value={form.moneyDeposit.accusedName}
          onChange={(v) => patch("moneyDeposit", "accusedName", v)}
        />
        <BmFieldText
          label="Tên vụ án"
          required
          value={form.moneyDeposit.caseTitle}
          onChange={(v) => patch("moneyDeposit", "caseTitle", v)}
        />
        <BmFieldText
          label="Tội danh"
          value={form.moneyDeposit.offenseName}
          onChange={(v) => patch("moneyDeposit", "offenseName", v)}
        />
        <BmFieldText
          label="Số tiền đặt bảo đảm"
          required
          value={form.moneyDeposit.bondAmount}
          onChange={(v) => patch("moneyDeposit", "bondAmount", v)}
        />
        <BmFieldTextarea
          label="Lý do áp dụng"
          required
          fullWidth
          value={form.moneyDeposit.reasonLine}
          onChange={(v) => patch("moneyDeposit", "reasonLine", v)}
          rows={3}
        />
