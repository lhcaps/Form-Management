 ...
 * Stage: KHOI_TO, Group: G02. TT 03/2026-VKSTC, Mẫu số 93/HS.
 *
 * Căn cứ: Điều 154, 156 BLTTHS.
 * Nghiệp vụ: VKS huỷ QĐ thay đổi QĐ khởi tố bị can đã ban hành.
 */
...
const EMPTY_FORM: Bm093Form = {
  ...
  document: { documentCode: "93/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
const REQUIRED_FIELDS: ReadonlyArray<[keyof Bm093Form, string, string]> = [
...
function buildIssuePlaceAndDateLine(form: Bm093Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm093Form): string {
  ...
  return `Xét thấy QĐ số ${d.cancelledDocumentCode.trim()}${dateText ? ` ${dateText}` : ""} của ${d.cancelledDocumentAgency.trim()} về việc thay đổi QĐ khởi tố bị can ${d.accusedFullName.trim()} trong vụ án ${d.caseTitle.trim()} về tội "${d.offenseName.trim()}" không có căn cứ và trái pháp luật;`;
}

function normalizeFormInputs(payload: RenderPayload | null): Bm093Form {
  const f = EMPTY_FORM;
  return {
    agency: {
...
function validateForm(form: Bm093Form): string[] {
...
function buildSaveBody(form: Bm093Form) {
...
export function Bm093FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm093Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm093Form, K extends keyof Bm093Form[S]>(
    section: S,
    key: K,
    value: Bm093Form[S][K],
  ) => {
