...
/**
 * BM-125 — Thông báo không chấp nhận đề nghị trưng cầu giám định, định giá tài sản
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 125/HS.
 *
 * Căn cứ: Điều 41, 211 BLTTHS 2015.
 */
...
const EMPTY_FORM: Bm125Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "125/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm125Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}



function normalizeFormInputs(payload: RenderPayload | null): Bm125Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    notification: {
      requestedAgency: nested(payload, "notification.requestedAgency") || "",
      requestCode: nested(payload, "notification.requestCode") || "",
      requestDate: nested(payload, "notification.requestDate") || "",
      requestContent: nested(payload, "notification.requestContent") || "",
      refusalReason: nested(payload, "notification.refusalReason") || "",
    },
    recipients: {
...
function lookupValue(form: Bm125Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm125Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm125Form) {
  return {
    agency: {
    ...
    notification: {
      requestedAgency: form.notification.requestedAgency,
      requestCode: form.notification.requestCode,
      requestDate: form.notification.requestDate,
      requestContent: form.notification.requestContent,
      refusalReason: form.notification.refusalReason,
...
export function Bm125FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm125Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm125Form, K extends keyof Bm125Form[S]>(
    section: S,
    key: K,
    value: Bm125Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-125 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-125 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-125"
        title="Dữ liệu biểu mẫu Thông báo không chấp nhận đề nghị trưng cầu giám định, định giá tài sản"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-125"}
      ...
      <BmFormSection title="2. Thông tin không chấp nhận" description="Nhập thông tin đề nghị không được chấp nhận." requiredCount={2}>
        <BmFieldText
          label="Cơ quan đề nghị"
          required
          value={form.notification.requestedAgency}
          onChange={(v) => patch("notification", "requestedAgency", v)}
        />
        <BmFieldText
          label="Số đề nghị"
          value={form.notification.requestCode}
          onChange={(v) => patch("notification", "requestCode", v)}
        />
        <BmFieldText
          label="Ngày đề nghị"
          value={form.notification.requestDate}
          onChange={(v) => patch("notification", "requestDate", v)}
        />
        <BmFieldTextarea
          label="Nội dung đề nghị"
          required
          fullWidth
          value={form.notification.requestContent}
          onChange={(v) => patch("notification", "requestContent", v)}
          rows={3}
        />
        <BmFieldTextarea
          label="Lý do không chấp nhận"
          required
          fullWidth
          value={form.notification.refusalReason}
          onChange={(v) => patch("notification", "refusalReason", v)}
          rows={3}
        />
