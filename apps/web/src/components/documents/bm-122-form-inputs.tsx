 ...
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 122/HS.
 *
 * Căn cứ: Điều 41, 116 BLTTHS 2015.
 */
...
const EMPTY_FORM: Bm122Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "122/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm122Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm122Form): string {
...
function normalizeFormInputs(payload: RenderPayload | null): Bm122Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    decision: {
      ...
      orderCode: nested(payload, "decision.orderCode") || "",
      orderDate: nested(payload, "decision.orderDate") || "",
      orderAgency: nested(payload, "decision.orderAgency") || "",
...
function lookupValue(form: Bm122Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm122Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm122Form) {
  return {
    agency: {
    ...
    decision: {
      ...
      orderCode: form.decision.orderCode,
      orderDate: form.decision.orderDate,
      orderAgency: form.decision.orderAgency,
...
export function Bm122FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm122Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm122Form, K extends keyof Bm122Form[S]>(
    section: S,
    key: K,
    value: Bm122Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-122 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-122 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-122"
        title="Dữ liệu biểu mẫu QĐ không phê chuẩn Lệnh thu giữ thư tín, điện tín, bưu kiện, bưu phẩm"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-122"}
      ...
      <BmFormSection title="2. Thông tin không phê chuẩn Lệnh" description="Nhập thông tin Lệnh không được phê chuẩn." requiredCount={2}>
        <BmFieldTextarea
        ...
        <BmFieldText
          label="Số Lệnh"
          required
          value={form.decision.orderCode}
          onChange={(v) => patch("decision", "orderCode", v)}
        />
        <BmFieldText
          label="Ngày Lệnh"
          value={form.decision.orderDate}
          onChange={(v) => patch("decision", "orderDate", v)}
        />
        <BmFieldText
          label="Cơ quan ban hành Lệnh"
          required
          value={form.decision.orderAgency}
          onChange={(v) => patch("decision", "orderAgency", v)}
        />
        <BmFieldText
          label="Mô tả đối tượng thu giữ"
