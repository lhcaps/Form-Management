...
/**
 * BM-116 — QĐ phê chuẩn lệnh thu giữ tài sản
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 116/HS.
 *
...
const EMPTY_FORM: Bm116Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "116/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm116Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm116Form): string {
...
function normalizeFormInputs(payload: RenderPayload | null): Bm116Form {
  const f = EMPTY_FORM;
  return {
    agency: {
...
function lookupValue(form: Bm116Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm116Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm116Form) {
  return {
    agency: {
...
export function Bm116FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm116Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm116Form, K extends keyof Bm116Form[S]>(
    section: S,
    key: K,
    value: Bm116Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-116 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-116 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-116"
        title="Dữ liệu biểu mẫu QĐ phê chuẩn lệnh thu giữ tài sản"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-116"}
      ...
      <BmFormSection title="2. Thông tin lệnh thu giữ" description="Nhập thông tin lệnh thu giữ tài sản được phê chuẩn." requiredCount={2}>
        <BmFieldTextarea
