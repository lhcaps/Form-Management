...
/**
 * BM-113 — QĐ kéo dài thời hạn tạm giam
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 113/HS.
 *
 * Căn cứ: Điều 41, 173 BLTTHS 2015.
 */
...
type DetentionExtensionForm = {
  detaineeName: string;
  caseTitle: string;
  offenseName: string;
  originalDetentionDecision: string;
...
type Bm113Form = {
  ...
  detentionExtension: DetentionExtensionForm;
...
function buildIssuePlaceAndDateLine(form: Bm113Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}



function normalizeFormInputs(payload: RenderPayload | null): Bm113Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    detentionExtension: {
      detaineeName: nested(payload, "detentionExtension.detaineeName") || "",
      caseTitle: nested(payload, "detentionExtension.caseTitle") || "",
      offenseName: nested(payload, "detentionExtension.offenseName") || "",
      originalDetentionDecision: nested(payload, "detentionExtension.originalDetentionDecision") || "",
      extendedPeriod: nested(payload, "detentionExtension.extendedPeriod") || "",
      reasonLine: nested(payload, "detentionExtension.reasonLine") || "",
    },
    recipients: {
...
function lookupValue(form: Bm113Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm113Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm113Form) {
  return {
    agency: {
    ...
    detentionExtension: {
      detaineeName: form.detentionExtension.detaineeName,
      caseTitle: form.detentionExtension.caseTitle,
      offenseName: form.detentionExtension.offenseName,
      originalDetentionDecision: form.detentionExtension.originalDetentionDecision,
      extendedPeriod: form.detentionExtension.extendedPeriod,
...
export function Bm113FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm113Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm113Form, K extends keyof Bm113Form[S]>(
    section: S,
    key: K,
    value: Bm113Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-113 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-113 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-113"
        title="Dữ liệu biểu mẫu QĐ kéo dài thời hạn tạm giam"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-113"}
      ...
      <BmFormSection title="2. Kéo dài thời hạn tạm giam" description="Thông tin kéo dài thời hạn tạm giam." requiredCount={2}>
        <BmFieldText
          label="Tên bị can bị tạm giam"
          required
          value={form.detentionExtension.detaineeName}
          onChange={(v) => patch("detentionExtension", "detaineeName", v)}
        />
        <BmFieldText
          label="Tên vụ án"
          required
          value={form.detentionExtension.caseTitle}
          onChange={(v) => patch("detentionExtension", "caseTitle", v)}
        />
        <BmFieldText
          label="Tội danh"
          value={form.detentionExtension.offenseName}
          onChange={(v) => patch("detentionExtension", "offenseName", v)}
        />
        <BmFieldText
          label="Số QĐ tạm giam ban đầu"
          value={form.detentionExtension.originalDetentionDecision}
          onChange={(v) => patch("detentionExtension", "originalDetentionDecision", v)}
        />
        <BmFieldText
          label="Thời hạn kéo dài"
          required
          value={form.detentionExtension.extendedPeriod}
          onChange={(v) => patch("detentionExtension", "extendedPeriod", v)}
        />
        <BmFieldTextarea
          label="Lý do kéo dài"
          required
          fullWidth
          value={form.detentionExtension.reasonLine}
          onChange={(v) => patch("detentionExtension", "reasonLine", v)}
          rows={3}
        />
