...
/**
 * BM-032 — Quyết định không phê chuẩn Lệnh bắt người bị giữ trong trường hợp khẩn cấp
...
const EMPTY_FORM: BM032FormInputs = {
  agency: {
  ...
  legalBasis: {
    procedureArticlesLine: "Căn cứ các điều 36, 41, 110 và 113 của Bộ luật Tố tụng hình sự;",
  },
  decision: {
    summaryLine: "",
    decisionLine: "",
...
function normalizeForm(source: unknown): BM032FormInputs {
...
function derive(form: BM032FormInputs): BM032FormInputs {
  return {
    ...form,
    document: {
      ...form.document,
...
export function Bm032FormInputsPanel({ documentId, onSaved }: Props) {
  const [form, setForm] = useState<BM032FormInputs>(EMPTY_FORM);
  ...
  function updateSection(sectionName: keyof BM032FormInputs, fieldName: string, value: string) {
    setForm((current) => {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setSavedAt(new Date());
      setSuccessMessage(`Đã lưu dữ liệu BM-032. Có thể render lại DOCX/PDF.`);
      onSaved?.();
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : `Lưu BM-032 thất bại`);
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function handleReload() {
    ...
    try {
      ...
      setInitialSnapshot(JSON.stringify(derive(next)));
      setSavedAt(null);
      setSuccessMessage(`Đã tải lại dữ liệu BM-032.`);
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : `Tải lại BM-032 thất bại`);
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-4">
      <BmFormMetaBar
        ...
        warningMessage={null}
        primaryLabel={`Lưu dữ liệu BM-032`}
        ...
        extraActions={
          <BmFormCasePayloadButton
            templateCode="BM-032"
            form={form}
            onApply={(next) => setForm(next as BM032FormInputs)}
          />
        }
      />
      ...
      <BmFormSection title="5. Nội dung quyết định">
        ...
        />
        <BmFieldTextarea
          label={`Nội dung quyết định (${"không phê chuẩn Lệnh bắt người bị giữ trong trường hợp khẩn cấp"})`}
