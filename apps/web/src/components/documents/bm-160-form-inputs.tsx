 ...
 * Stage: PHUC_HOI_XAC_MINH, Group: G99. TT 03/2026-VKSTC, Mẫu số 160/HS.
 *
 * Căn cứ: Điều 43, 44 BLTTHS 2015.
 * Nghiệp vụ: VKS lập biên bản niêm yết công khai văn bản tố tụng.
 */
...
function normalizeForm(payload: RenderPayload | null): Bm160Form {
  ...
  return {
    ...
    content: {
      postingLocation: nested(payload, "content.postingLocation") || "",
      postingDate: nested(payload, "content.postingDate") || "",
      postingTime: nested(payload, "content.postingTime") || "08:00",
      postingContent: nested(payload, "content.postingContent") || "",
...
function buildSaveBody(form: Bm160Form) {
  return {
    agency: form.agency,
...
export function Bm160FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm160Form>(EMPTY_FORM);
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  const patch = <S extends keyof Bm160Form, K extends keyof Bm160Form[S]>(
    section: S,
    key: K,
    value: Bm160Form[S][K],
  ) => {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-160"
        title="Dữ liệu biểu mẫu Biên bản niêm yết công khai văn bản tố tụng"
        subtitle="Biểu mẫu TT 03/2026-VKSTC · Mẫu số 160/HS · Căn cứ Điều 43, 44 BLTTHS 2015."
        isDirty={false}
        isLoading={loading}
        isSaving={saving}
        savedAt={null}
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-160"}
      ...
      <BmFormSection title="2. Thông tin niêm yết">
        <BmFieldText
          label="Địa điểm niêm yết"
          fullWidth
          value={form.content.postingLocation}
          onChange={(v) => patch("content", "postingLocation", v)}
        />
        <BmFieldDate
          label="Ngày niêm yết"
          value={form.content.postingDate}
          onChange={(v) => patch("content", "postingDate", v)}
        />
        <BmFieldText
          label="Giờ niêm yết"
          value={form.content.postingTime}
          placeholder="HH:MM"
          onChange={(v) => patch("content", "postingTime", v)}
        />
        <BmFieldTextarea
          label="Nội dung văn bản được niêm yết"
          fullWidth
          value={form.content.postingContent}
          onChange={(v) => patch("content", "postingContent", v)}
          rows={3}
        />
        <BmFieldTextarea
          label="Điều 1"
      ...
      <BmFormSection title="3. Nơi nhận">
        <BmFieldText
          label="Nơi nhận 1"
