...
/**
 * BM-105 — QĐ không gia hạn thời hạn điều tra vụ án hình sự
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 105/HS.
 *
 * Căn cứ: Điều 36, 37, 172 BLTTHS 2015.
 * Nghiệp vụ: VKS quyết định không gia hạn thời hạn điều tra vụ án HS.
 */
...
const EMPTY_FORM: Bm105Form = {
  ...
  document: { documentCode: "105/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
  official: { issuerTitle: "VIỆN TRƯỞNG VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7" },
  decision: {
    procedureArticlesLine: "Căn cứ các điều 36, 37 và 172 của Bộ luật Tố tụng hình sự;",
    caseTitle: "",
    offenseName: "",
...
function buildIssuePlaceAndDateLine(form: Bm105Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm105Form): string {
  const d = form.decision;
  if (!d.caseTitle) return "";
  return `Xét thấy thời hạn điều tra vụ án ${d.caseTitle.trim()} về tội "${d.offenseName.trim()}" đã hết mà không thể kết thúc điều tra${d.investigationDeadline ? ` (thời hạn: ${d.investigationDeadline.trim()})` : ""}. Không có căn cứ để gia hạn thời hạn điều tra.`;
}

function normalizeFormInputs(payload: RenderPayload | null): Bm105Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    decision: {
      ...
      caseTitle: nested(payload, "decision.caseTitle") || nested(payload, "case.caseTitle") || "",
      offenseName: nested(payload, "decision.offenseName") || nested(payload, "offense.offenseName") || "",
...
function lookupValue(form: Bm105Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm105Form): string[] {
  return REQUIRED_FIELDS.filter(([, path]) => !lookupValue(form, path)).map(
    ([label]) => label,
  );
}

function buildSaveBody(form: Bm105Form) {
  return {
    agency: {
...
export function Bm105FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm105Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm105Form, K extends keyof Bm105Form[S]>(
    section: S,
    key: K,
    value: Bm105Form[S][K],
  ) => {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-105"
        title="Dữ liệu biểu mẫu QĐ không gia hạn thời hạn điều tra VAHS"
        subtitle="Biểu mẫu TT 03/2026-VKSTC · Mẫu số 105/HS · Căn cứ Điều 36, 37, 172 BLTTHS 2015."
        isDirty={false}
        isLoading={loading}
        isSaving={saving}
        savedAt={null}
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-105"}
      ...
      <BmFormSection
        ...
        description="Nhập thông tin vụ án và thời hạn điều tra."
        requiredCount={2}
      >
        <BmFieldTextarea
