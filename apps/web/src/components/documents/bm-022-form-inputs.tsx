...
/**
 * BM-022 — Quyết định huỷ bỏ Quyết định không khởi tố vụ án hình sự
...
const EMPTY_FORM: BM022FormInputs = {
  agency: {
  ...
  legalBasis: {
    procedureArticlesLine: "Căn cứ các điều 36, 39, 148 và 149 của Bộ luật Tố tụng hình sự;",
  },
  decision: {
    summaryLine: "",
    decisionLine: "",
...
function normalizeForm(source: unknown): BM022FormInputs {
...
function derive(form: BM022FormInputs): BM022FormInputs {
  return {
    ...form,
    document: {
      ...form.document,
...
export function Bm022FormInputsPanel({ documentId, onSaved }: Props) {
  const [form, setForm] = useState<BM022FormInputs>(EMPTY_FORM);
  ...
  function updateSection(sectionName: keyof BM022FormInputs, fieldName: string, value: string) {
    setForm((current) => {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setSavedAt(new Date());
      setSuccessMessage(`Đã lưu dữ liệu BM-022. Có thể render lại DOCX/PDF.`);
      onSaved?.();
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : `Lưu BM-022 thất bại`);
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function handleReload() {
    ...
    try {
      ...
      setInitialSnapshot(JSON.stringify(derive(next)));
      setSavedAt(null);
      setSuccessMessage(`Đã tải lại dữ liệu BM-022.`);
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : `Tải lại BM-022 thất bại`);
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-4">
      <BmFormMetaBar
        ...
        warningMessage={null}
        primaryLabel={`Lưu dữ liệu BM-022`}
        ...
        extraActions={
          <BmFormCasePayloadButton
            templateCode="BM-022"
            form={form}
            onApply={(next) => setForm(next as BM022FormInputs)}
          />
        }
      />
      ...
      <BmFormSection title="5. Nội dung quyết định">
        ...
        />
        <BmFieldTextarea
          label={`Nội dung quyết định (${"huỷ bỏ Quyết định không khởi tố vụ án hình sự"})`}
