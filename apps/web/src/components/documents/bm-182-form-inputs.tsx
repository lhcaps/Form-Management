 ...
 * Stage: THU_TUC_DAC_BIET (Thủ tục đặc biệt, rút gọn), Group: G04
 *
...
export function Bm182FormInputsPanel({ documentId, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setIsLoading(true);
        const res = await getDocumentRenderPayload<PayloadResponse>(documentId);
        if (isMounted) setPayload(res);
      } catch (e) {
  ...
  return (
    <div className="space-y-4">
      <BmFormMetaBar
        ...
        subtitle={`Biểu mẫu TT 03/2026-VKSTC · Stage: THU_TUC_DAC_BIET (Thủ tục đặc biệt, rút gọn) · Group: G04 · Scope: ${scope}`}
        templateCode="BM-182"
        isDirty={isDirty}
        isLoading={isLoading}
        errorMessage={error}
        ...
        meta={
          <div className="text-xs text-slate-500">
            <div>Stage: <span className="font-mono">THU_TUC_DAC_BIET</span></div>
