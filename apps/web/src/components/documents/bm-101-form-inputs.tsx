...
/**
 * BM-101 — QĐ phê chuẩn QĐ bổ sung QĐ khởi tố bị can
 * Stage: KHOI_TO, Group: G02. TT 03/2026-VKSTC, Mẫu số 101/HS.
 *
...
type Bm101Form = {
  ...
  decisionSupplement: DecisionSupplementForm;
...
const EMPTY_FORM: Bm101Form = {
  agency: {
    parentName: "",
    name: "",
  },
  document: {
    documentCode: "101/QĐ-VKSKV7",
...
const REQUIRED_FIELDS: ReadonlyArray<[keyof Bm101Form, string, string]> = [
  ...
  ["decisionSupplement", "procedureArticlesLine", "Căn cứ tố tụng"],
  ["decisionSupplement", "oldDecisionInfoLine", "Quyết định cũ"],
  ["decisionSupplement", "reasonLine", "Lý do thay đổi"],
  ["decisionSupplement", "personName", "Tên bị can"],
  ["decisionSupplement", "offenseName", "Tội danh"],
...
function buildIssuePlaceAndDateLine(form: Bm101Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm101Form): string {
  const data = form.decisionSupplement;
...
function buildNewDecisionLine(form: Bm101Form): string {
  const data = form.decisionSupplement;
...
function normalizeFormInputs(payload: RenderPayload | null): Bm101Form {
  ...
  return {
    ...
    decisionSupplement: {
      procedureArticlesLine:
        nested(payload, "decisionSupplement.procedureArticlesLine") ||
        EMPTY_FORM.decisionSupplement.procedureArticlesLine,
      oldDecisionInfoLine:
        nested(payload, "decisionSupplement.oldDecisionInfoLine") ||
        EMPTY_FORM.decisionSupplement.oldDecisionInfoLine,
      reasonLine:
        nested(payload, "decisionSupplement.reasonLine") ||
        EMPTY_FORM.decisionSupplement.reasonLine,
      personName:
        nested(payload, "decisionSupplement.personName") ||
        nested(payload, "person.fullName") ||
        EMPTY_FORM.decisionSupplement.personName,
      offenseName:
        nested(payload, "decisionSupplement.offenseName") ||
        nested(payload, "offense.offenseName") ||
        EMPTY_FORM.decisionSupplement.offenseName,
      newDecisionLine:
        nested(payload, "decisionSupplement.newDecisionLine") ||
        EMPTY_FORM.decisionSupplement.newDecisionLine,
    },
    recipients: {
...
function validateForm(form: Bm101Form): string[] {
...
function buildSaveBody(form: Bm101Form) {
  ...
  const decisionSupplement = {
    procedureArticlesLine: form.decisionSupplement.procedureArticlesLine,
    oldDecisionInfoLine: form.decisionSupplement.oldDecisionInfoLine,
    reasonLine: buildReasonLine(form),
    personName: form.decisionSupplement.personName,
    offenseName: form.decisionSupplement.offenseName,
...
export function Bm101FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm101Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm101Form, K extends keyof Bm101Form[S]>(
    section: S,
    key: K,
    value: Bm101Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải lại dữ liệu BM-101 từ backend.");
    } catch (err) {
  ...
  const handleFillSample = () => {
    setForm({
      ...
      decisionSupplement: { ...EMPTY_FORM.decisionSupplement },
      recipients: { ...EMPTY_FORM.recipients },
      signature: { ...EMPTY_FORM.signature },
    });
    setError(null);
    setMessage("Đã điền dữ liệu mẫu BM-101.");
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu dữ liệu BM-101. Các dòng tự sinh đã đồng bộ.");
      await onSaved?.();
    } catch (err) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-101"}
        ...
        extraActions={
          <>
            <BmFormCasePayloadButton<Bm101Form>
              templateCode="BM-101"
              form={form}
              onApply={(next) => {
                setForm(next);
      ...
      >
        <BmFieldTextarea
          ...
          required
          value={form.decisionSupplement.procedureArticlesLine}
          onChange={(v) => patch("decisionSupplement", "procedureArticlesLine", v)}
          rows={2}
        />
        <BmFieldText
          label="Quyết định khởi tố bị can cũ"
          required
          value={form.decisionSupplement.oldDecisionInfoLine}
          onChange={(v) => patch("decisionSupplement", "oldDecisionInfoLine", v)}
        />
        <BmFieldTextarea
          label="Lý do thay đổi"
          required
          value={form.decisionSupplement.reasonLine}
          onChange={(v) => patch("decisionSupplement", "reasonLine", v)}
          rows={3}
        />
        <BmFieldText
          label="Tên bị can"
          required
          value={form.decisionSupplement.personName}
          onChange={(v) => patch("decisionSupplement", "personName", v)}
        />
        <BmFieldText
          label="Tội danh"
          required
          value={form.decisionSupplement.offenseName}
          onChange={(v) => patch("decisionSupplement", "offenseName", v)}
        />
        <BmFieldTextarea
          label="Nội dung quyết định thay đổi (tự sinh nếu trống)"
          value={form.decisionSupplement.newDecisionLine}
          onChange={(v) => patch("decisionSupplement", "newDecisionLine", v)}
          rows={3}
        />
      </BmFormSection>
