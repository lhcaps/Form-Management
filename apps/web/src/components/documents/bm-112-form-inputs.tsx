...
/**
 * BM-112 — QĐ thay đổi biện pháp tạm giữ
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 112/HS.
 *
 * Căn cứ: Điều 41, 115 BLTTHS 2015.
 */
...
const EMPTY_FORM: Bm112Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "112/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm112Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm112Form): string {
...
function normalizeFormInputs(payload: RenderPayload | null): Bm112Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    decision: {
      detaineeName: nested(payload, "decision.detaineeName") || "",
      caseTitle: nested(payload, "decision.caseTitle") || "",
      offenseName: nested(payload, "decision.offenseName") || "",
      oldDecisionCode: nested(payload, "decision.oldDecisionCode") || "",
      newMeasure: nested(payload, "decision.newMeasure") || "",
...
function lookupValue(form: Bm112Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm112Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm112Form) {
  return {
    agency: {
    ...
    decision: {
      detaineeName: form.decision.detaineeName,
      caseTitle: form.decision.caseTitle,
      offenseName: form.decision.offenseName,
      oldDecisionCode: form.decision.oldDecisionCode,
      newMeasure: form.decision.newMeasure,
...
export function Bm112FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm112Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm112Form, K extends keyof Bm112Form[S]>(
    section: S,
    key: K,
    value: Bm112Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-112 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-112 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-112"
        title="Dữ liệu biểu mẫu QĐ thay đổi biện pháp tạm giữ"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-112"}
      ...
      <BmFormSection title="2. Thông tin thay đổi tạm giữ" description="Nhập thông tin thay đổi biện pháp tạm giữ." requiredCount={2}>
        <BmFieldText
          label="Tên người bị tạm giữ"
          required
          value={form.decision.detaineeName}
          onChange={(v) => patch("decision", "detaineeName", v)}
        />
        <BmFieldText
          label="Tên vụ án"
          required
        ...
        />
        <BmFieldText
          label="Số QĐ tạm giữ cũ"
          required
          value={form.decision.oldDecisionCode}
          onChange={(v) => patch("decision", "oldDecisionCode", v)}
        />
        <BmFieldText
          label="Biện pháp mới"
          required
          value={form.decision.newMeasure}
          onChange={(v) => patch("decision", "newMeasure", v)}
        />
        <BmFieldTextarea
