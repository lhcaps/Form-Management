...
/**
 * BM-114 — QĐ thay đổi người bảo vệ
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 114/HS.
 *
 * Căn cứ: Điều 41, 76 BLTTHS 2015.
 */
...
type DefenderForm = {
  accusedName: string;
  caseTitle: string;
  oldDefenderName: string;
  newDefenderName: string;
  reasonLine: string;
};
...
const EMPTY_FORM: Bm114Form = {
  ...
  document: { documentCode: "114/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function normalizeFormInputs(payload: RenderPayload | null): Bm114Form {
  ...
  return {
    ...
    defender: {
      accusedName: nested(payload, "defender.accusedName") || "",
      caseTitle: nested(payload, "defender.caseTitle") || "",
      oldDefenderName: nested(payload, "defender.oldDefenderName") || "",
      newDefenderName: nested(payload, "defender.newDefenderName") || "",
      reasonLine: nested(payload, "defender.reasonLine") || "",
    },
    recipients: {
...
function lookupValue(form: Bm114Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function buildSaveBody(form: Bm114Form) {
  return {
    ...
    defender: {
      accusedName: form.defender.accusedName,
      caseTitle: form.defender.caseTitle,
      oldDefenderName: form.defender.oldDefenderName,
      newDefenderName: form.defender.newDefenderName,
...
export function Bm114FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const patch = <S extends keyof Bm114Form, K extends keyof Bm114Form[S]>(
    section: S,
    key: K,
    value: Bm114Form[S][K],
  ) => {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-114"
        title="Dữ liệu biểu mẫu QĐ thay đổi người bảo vệ"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-114"}
      ...
      <BmFormSection title="2. Thông tin thay đổi người bảo vệ" description="Nhập thông tin thay đổi người bảo vệ." requiredCount={2}>
        <BmFieldText
          label="Tên bị can/bị cáo"
          required
          value={form.defender.accusedName}
          onChange={(v) => patch("defender", "accusedName", v)}
        />
        <BmFieldText
          label="Tên vụ án"
          required
          value={form.defender.caseTitle}
          onChange={(v) => patch("defender", "caseTitle", v)}
        />
        <BmFieldText
          label="Tên người bảo vệ cũ"
          required
          value={form.defender.oldDefenderName}
          onChange={(v) => patch("defender", "oldDefenderName", v)}
        />
        <BmFieldText
          label="Tên người bảo vệ mới"
          required
          value={form.defender.newDefenderName}
          onChange={(v) => patch("defender", "newDefenderName", v)}
        />
        <BmFieldTextarea
          label="Lý do thay đổi"
          required
          fullWidth
          value={form.defender.reasonLine}
          onChange={(v) => patch("defender", "reasonLine", v)}
          rows={3}
        />
