...
/**
 * BM-152 — Quyết định đình chỉ vụ án đối với bị can
 * Stage: TRUY_TO, Group: G05. TT 03/2026-VKSTC, Mẫu số 152/HS.
 *
 * Căn cứ: Căn cứ Điều 41 và Điều 248 của Bộ luật Tố tụng hình sự;
...
type LegalBasisForm = {
  ...
  caseDecisionLine: string;
  accusedDecisionLine: string;
};
type ContentForm = {
  accusedName: string;
  offenseName: string;
...
const EMPTY_FORM: Bm152Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN THÀNH PHỐ HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
    issuePlace: "TP. Hồ Chí Minh",
  },
  document: { documentCode: "QĐ-VKSKV7-DINHCHI", issueDateIso: "" },
  official: {
    issuerTitle: "VIỆN TRƯỞNG VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  legalBasis: {
  procedureArticlesLine: 'Căn cứ Điều 41 và Điều 248 của Bộ luật Tố tụng hình sự;',
    caseDecisionLine: "",
    accusedDecisionLine: "",
  },
  content: {
...
const REQUIRED_FIELDS: ReadonlyArray<[string, string]> =   [
    ...
    ["Căn cứ QĐ khởi tố vụ án", "legalBasis.caseDecisionLine"],
    ["Căn cứ QĐ khởi tố bị can", "legalBasis.accusedDecisionLine"],
    ["Tên vụ án", "content.caseName"],
    ["Lý do đình chỉ", "content.reasonLine"],
...
function buildIssuePlaceAndDateLine(form: Bm152Form): string {
  return issuePlaceDateLine(form.agency.issuePlace, form.document.issueDateIso);
}

function normalizeFormInputs(payload: RenderPayload | null): Bm152Form {
  const f = EMPTY_FORM;
  if (!payload) return f;
  return {
    agency: {
    ...
    legalBasis: {
        ...
        caseDecisionLine: nested(payload, "legalBasis.caseDecisionLine") || "",
        accusedDecisionLine: nested(payload, "legalBasis.accusedDecisionLine") || "",
    },
    content: {
...
function lookupValue(form: Bm152Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
...
function validateForm(form: Bm152Form): string[] {
  return REQUIRED_FIELDS.filter(([, path]) => !lookupValue(form, path)).map(
    ([label]) => label,
  );
}

function buildSaveBody(form: Bm152Form) {
  return {
    agency: {
    ...
    legalBasis: {
      ...
      caseDecisionLine: form.legalBasis.caseDecisionLine,
      accusedDecisionLine: form.legalBasis.accusedDecisionLine,
    },
    content: {
      accusedName: form.content.accusedName,
      offenseName: form.content.offenseName,
      legalArticle: form.content.legalArticle,
...
export function Bm152FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm152Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm152Form, K extends keyof Bm152Form[S]>(
    section: S,
    key: K,
    value: Bm152Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-152 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-152 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-152"
        title="Dữ liệu biểu mẫu Quyết định đình chỉ vụ án đối với bị can"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-152"}
      ...
      >
        ...
        <BmFieldTextarea
                  ...
                  fullWidth
                  value={form.legalBasis.caseDecisionLine}
                  onChange={(v) => patch("legalBasis", "caseDecisionLine", v)}
                  rows={2}
                />
        <BmFieldTextarea
                  label="Căn cứ QĐ khởi tố bị can"
                  fullWidth
                  value={form.legalBasis.accusedDecisionLine}
                  onChange={(v) => patch("legalBasis", "accusedDecisionLine", v)}
                  rows={2}
                />
      </BmFormSection>
