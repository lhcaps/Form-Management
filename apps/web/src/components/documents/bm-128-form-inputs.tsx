...
/**
 * BM-128 — QĐ hủy bỏ biện pháp ngăn chặn
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 128/HS.
 *
 * Căn cứ: Điều 41, 119 BLTTHS 2015.
 */
...
type MeasureCancellationForm = {
  accusedName: string;
  caseTitle: string;
  offenseName: string;
  cancelledMeasure: string;
  reasonLine: string;
};
...
type Bm128Form = {
  ...
  measureCancellation: MeasureCancellationForm;
...
const EMPTY_FORM: Bm128Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "128/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm128Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}



function normalizeFormInputs(payload: RenderPayload | null): Bm128Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    measureCancellation: {
      accusedName: nested(payload, "measureCancellation.accusedName") || "",
      caseTitle: nested(payload, "measureCancellation.caseTitle") || "",
      offenseName: nested(payload, "measureCancellation.offenseName") || "",
      cancelledMeasure: nested(payload, "measureCancellation.cancelledMeasure") || "",
      reasonLine: nested(payload, "measureCancellation.reasonLine") || "",
    },
    recipients: {
...
function lookupValue(form: Bm128Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm128Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm128Form) {
  return {
    agency: {
    ...
    measureCancellation: {
      accusedName: form.measureCancellation.accusedName,
      caseTitle: form.measureCancellation.caseTitle,
      offenseName: form.measureCancellation.offenseName,
      cancelledMeasure: form.measureCancellation.cancelledMeasure,
...
export function Bm128FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm128Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm128Form, K extends keyof Bm128Form[S]>(
    section: S,
    key: K,
    value: Bm128Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-128 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-128 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-128"
        title="Dữ liệu biểu mẫu QĐ hủy bỏ biện pháp ngăn chặn"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-128"}
      ...
      <BmFormSection title="2. Huỷ bỏ biện pháp ngăn chặn" description="Thông tin biện pháp ngăn chặn bị huỷ bỏ." requiredCount={2}>
        <BmFieldText
          label="Tên bị can/bị cáo"
          required
          value={form.measureCancellation.accusedName}
          onChange={(v) => patch("measureCancellation", "accusedName", v)}
        />
        <BmFieldText
          label="Tên vụ án"
          required
          value={form.measureCancellation.caseTitle}
          onChange={(v) => patch("measureCancellation", "caseTitle", v)}
        />
        <BmFieldText
          label="Tội danh"
          value={form.measureCancellation.offenseName}
          onChange={(v) => patch("measureCancellation", "offenseName", v)}
        />
        <BmFieldText
          label="Biện pháp bị huỷ"
          required
          value={form.measureCancellation.cancelledMeasure}
          onChange={(v) => patch("measureCancellation", "cancelledMeasure", v)}
        />
        <BmFieldTextarea
          label="Lý do huỷ bỏ"
          required
          fullWidth
          value={form.measureCancellation.reasonLine}
          onChange={(v) => patch("measureCancellation", "reasonLine", v)}
          rows={3}
        />
