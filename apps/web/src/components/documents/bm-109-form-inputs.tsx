...
/**
 * BM-109 — QĐ huỷ bỏ QĐ phục hồi điều tra VAHS
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 109/HS.
 *
...
const EMPTY_FORM: Bm109Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "109/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm109Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm109Form): string {
...
function normalizeFormInputs(payload: RenderPayload | null): Bm109Form {
  const f = EMPTY_FORM;
  return {
    agency: {
...
function lookupValue(form: Bm109Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm109Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm109Form) {
  return {
    agency: {
...
export function Bm109FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm109Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm109Form, K extends keyof Bm109Form[S]>(
    section: S,
    key: K,
    value: Bm109Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-109 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-109 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-109"
        title="Dữ liệu biểu mẫu QĐ huỷ bỏ QĐ phục hồi điều tra VAHS"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-109"}
      ...
      <BmFormSection title="2. Quyết định bị huỷ bỏ" description="Thông tin QĐ phục hồi điều tra VAHS cần huỷ bỏ." requiredCount={2}>
        <BmFieldText
          label="Số QĐ bị huỷ"
          required
