...
/**
 * BM-134 — QĐ thu hồi tài sản đã kê biên
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 134/HS.
 *
...
type DecisionForm = {
  ...
  seizureDecisionCode: string;
  seizureDate: string;
  seizureAgency: string;
...
const EMPTY_FORM: Bm134Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "134/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm134Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm134Form): string {
...
function normalizeFormInputs(payload: RenderPayload | null): Bm134Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    decision: {
      ...
      seizureDecisionCode: nested(payload, "decision.seizureDecisionCode") || "",
      seizureDate: nested(payload, "decision.seizureDate") || "",
      seizureAgency: nested(payload, "decision.seizureAgency") || "",
      returnToPersonName: nested(payload, "decision.returnToPersonName") || "",
    },
    recipients: {
...
function lookupValue(form: Bm134Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm134Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm134Form) {
  return {
    agency: {
    ...
    decision: {
      ...
      seizureDecisionCode: form.decision.seizureDecisionCode,
      seizureDate: form.decision.seizureDate,
      seizureAgency: form.decision.seizureAgency,
      returnToPersonName: form.decision.returnToPersonName,
...
export function Bm134FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm134Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm134Form, K extends keyof Bm134Form[S]>(
    section: S,
    key: K,
    value: Bm134Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-134 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-134 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-134"
        title="Dữ liệu biểu mẫu QĐ thu hồi tài sản đã kê biên"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-134"}
      ...
      <BmFormSection title="2. Thông tin thu hồi tài sản" description="Nhập thông tin tài sản đã kê biên." requiredCount={2}>
        <BmFieldTextarea
        ...
        />
        <BmFieldTextarea
          label="Mô tả tài sản đã kê biên"
          required
          fullWidth
        ...
        <BmFieldText
          label="Số QĐ kê biên"
          required
          value={form.decision.seizureDecisionCode}
          onChange={(v) => patch("decision", "seizureDecisionCode", v)}
        />
        <BmFieldDate
          label="Ngày kê biên"
          value={form.decision.seizureDate}
          onChange={(v) => patch("decision", "seizureDate", v)}
        />
        <BmFieldText
          label="Cơ quan kê biên"
          value={form.decision.seizureAgency}
          onChange={(v) => patch("decision", "seizureAgency", v)}
        />
        <BmFieldText
          label="Người nhận lại"
          required
          value={form.decision.returnToPersonName}
          onChange={(v) => patch("decision", "returnToPersonName", v)}
        />
        <BmFieldTextarea
          label="Lý do tự sinh"
          fullWidth
