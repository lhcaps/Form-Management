 ...
 * Stage: NGUOI_THAM_GIA (Người tham gia tố tụng), Group: G02
 *
...
export function Bm075FormInputsPanel({ documentId, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setIsLoading(true);
        const data = await getDocumentRenderPayload<PayloadResponse>(documentId);
  ...
  return (
    <div className="space-y-4">
      <BmFormMetaBar
        ...
        subtitle={`Biểu mẫu TT 03/2026-VKSTC · Stage: NGUOI_THAM_GIA (Người tham gia tố tụng) · Group: G02 · Scope: ${scope}`}
        templateCode="BM-075"
        isDirty={isDirty}
        isLoading={isLoading}
        errorMessage={error}
        ...
        meta={
          <div className="text-xs text-slate-500">
            <div>Stage: <span className="font-mono">NGUOI_THAM_GIA</span></div>
            <div>Group: <span className="font-mono">G02</span></div>
          </div>
        }
      />
