 ...
 * Stage: KHOI_TO, Group: G02. TT 03/2026-VKSTC, Mẫu số 91/HS.
 *
 * Căn cứ: Điều 36, 176, 177 BLTTHS.
 * Nghiệp vụ: VKS phê chuẩn QĐ thay đổi QĐ khởi tố bị can do CQĐT ban hành.
 */
...
type DecisionAmendmentApprovalForm = {
...
type Bm091Form = {
  ...
  decisionAmendmentApproval: DecisionAmendmentApprovalForm;
...
const EMPTY_FORM: Bm091Form = {
  agency: {
    parentName: "",
    name: "",
  },
  document: {
    documentCode: "91/QĐ-VKSKV7",
...
const REQUIRED_FIELDS: ReadonlyArray<[keyof Bm091Form, string, string]> = [
...
function buildIssuePlaceAndDateLine(form: Bm091Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}

function buildReasonLine(form: Bm091Form): string {
...
function buildNewDecisionLine(form: Bm091Form): string {
  const data = form.decisionAmendmentApproval;
...
function normalizeFormInputs(payload: RenderPayload | null): Bm091Form {
  ...
  return {
    ...
    decisionAmendmentApproval: {
      procedureArticlesLine:
        nested(payload, "decisionAmendmentApproval.procedureArticlesLine") ||
        EMPTY_FORM.decisionAmendmentApproval.procedureArticlesLine,
      oldDecisionInfoLine:
        nested(payload, "decisionAmendmentApproval.oldDecisionInfoLine") ||
        EMPTY_FORM.decisionAmendmentApproval.oldDecisionInfoLine,
      reasonLine:
        nested(payload, "decisionAmendmentApproval.reasonLine") ||
        EMPTY_FORM.decisionAmendmentApproval.reasonLine,
      personName:
        nested(payload, "decisionAmendmentApproval.personName") ||
        nested(payload, "person.fullName") ||
        EMPTY_FORM.decisionAmendmentApproval.personName,
      offenseName:
        nested(payload, "decisionAmendmentApproval.offenseName") ||
        nested(payload, "offense.offenseName") ||
        EMPTY_FORM.decisionAmendmentApproval.offenseName,
      newDecisionLine:
        nested(payload, "decisionAmendmentApproval.newDecisionLine") ||
        EMPTY_FORM.decisionAmendmentApproval.newDecisionLine,
    },
    recipients: {
...
function validateForm(form: Bm091Form): string[] {
...
function buildSaveBody(form: Bm091Form) {
  ...
  const decisionAmendmentApproval = {
    procedureArticlesLine: form.decisionAmendmentApproval.procedureArticlesLine,
    oldDecisionInfoLine: form.decisionAmendmentApproval.oldDecisionInfoLine,
    reasonLine: buildReasonLine(form),
    personName: form.decisionAmendmentApproval.personName,
    offenseName: form.decisionAmendmentApproval.offenseName,
...
export function Bm091FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm091Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm091Form, K extends keyof Bm091Form[S]>(
    section: S,
    key: K,
    value: Bm091Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải lại dữ liệu BM-091 từ backend.");
    } catch (err) {
  ...
  const handleFillSample = () => {
    setForm({
      ...
      decisionAmendmentApproval: { ...EMPTY_FORM.decisionAmendmentApproval },
      recipients: { ...EMPTY_FORM.recipients },
      signature: { ...EMPTY_FORM.signature },
    });
    setError(null);
    setMessage("Đã điền dữ liệu mẫu BM-091.");
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu dữ liệu BM-091. Các dòng tự sinh đã đồng bộ.");
      await onSaved?.();
    } catch (err) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-091"}
        ...
        extraActions={
          <>
            <BmFormCasePayloadButton<Bm091Form>
              templateCode="BM-091"
              form={form}
              onApply={(next) => {
                setForm(next);
      ...
      >
        <BmFieldTextarea
          ...
          required
          value={form.decisionAmendmentApproval.procedureArticlesLine}
        ...
        <BmFieldText
          ...
          required
          value={form.decisionAmendmentApproval.oldDecisionInfoLine}
          onChange={(v) => patch("decisionAmendmentApproval", "oldDecisionInfoLine", v)}
        />
        <BmFieldTextarea
          label="Lý do thay đổi"
          required
          value={form.decisionAmendmentApproval.reasonLine}
