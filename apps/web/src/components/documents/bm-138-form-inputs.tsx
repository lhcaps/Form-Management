...
/**
 * BM-138 — QĐ áp dụng biện pháp cấm đi khỏi nơi cư trú
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 138/HS.
 *
 * Căn cứ: Điều 41, 123 BLTTHS 2015.
 */
...
const EMPTY_FORM: Bm138Form = {
  agency: {
    parentName: "VIỆN KIỂM SÁT NHÂN DÂN CẤP CAO TẠI TP. HỒ CHÍ MINH",
    name: "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7",
  },
  document: { documentCode: "138/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
function buildIssuePlaceAndDateLine(form: Bm138Form): string {
  return issuePlaceDateLine(form.document.issuePlace, form.document.issueDateIso);
}



function normalizeFormInputs(payload: RenderPayload | null): Bm138Form {
  const f = EMPTY_FORM;
  return {
    agency: {
    ...
    residenceBan: {
      accusedName: nested(payload, "residenceBan.accusedName") || "",
      caseTitle: nested(payload, "residenceBan.caseTitle") || "",
      offenseName: nested(payload, "residenceBan.offenseName") || "",
      residenceAddress: nested(payload, "residenceBan.residenceAddress") || "",
      restrictionPeriod: nested(payload, "residenceBan.restrictionPeriod") || "",
      reasonLine: nested(payload, "residenceBan.reasonLine") || "",
    },
    recipients: {
...
function lookupValue(form: Bm138Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function validateForm(form: Bm138Form): string[] {
  return REQUIRED_FIELDS.filter(([, p]) => !lookupValue(form, p)).map(([l]) => l);
}

function buildSaveBody(form: Bm138Form) {
  return {
    agency: {
    ...
    residenceBan: {
      accusedName: form.residenceBan.accusedName,
      caseTitle: form.residenceBan.caseTitle,
      offenseName: form.residenceBan.offenseName,
      residenceAddress: form.residenceBan.residenceAddress,
      restrictionPeriod: form.residenceBan.restrictionPeriod,
...
export function Bm138FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const [form, setForm] = useState<Bm138Form>(EMPTY_FORM);
  ...
  const patch = <S extends keyof Bm138Form, K extends keyof Bm138Form[S]>(
    section: S,
    key: K,
    value: Bm138Form[S][K],
  ) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-138 từ backend.");
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-138 thành công.");
      await onSaved?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-138"
        title="Dữ liệu biểu mẫu QĐ áp dụng biện pháp cấm đi khỏi nơi cư trú"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-138"}
      ...
      <BmFormSection title="2. Áp dụng biện pháp cấm đi khỏi nơi cư trú" description="Thông tin biện pháp cấm đi khỏi nơi cư trú." requiredCount={2}>
        <BmFieldText
          label="Tên bị can/bị cáo"
          required
          value={form.residenceBan.accusedName}
          onChange={(v) => patch("residenceBan", "accusedName", v)}
        />
        <BmFieldText
          label="Tên vụ án"
          required
          value={form.residenceBan.caseTitle}
          onChange={(v) => patch("residenceBan", "caseTitle", v)}
        />
        <BmFieldText
          label="Tội danh"
          value={form.residenceBan.offenseName}
          onChange={(v) => patch("residenceBan", "offenseName", v)}
        />
        <BmFieldText
          label="Nơi cư trú"
          required
          value={form.residenceBan.residenceAddress}
          onChange={(v) => patch("residenceBan", "residenceAddress", v)}
        />
        <BmFieldText
          label="Thời hạn cấm"
          required
          value={form.residenceBan.restrictionPeriod}
          onChange={(v) => patch("residenceBan", "restrictionPeriod", v)}
        />
        <BmFieldTextarea
          label="Lý do áp dụng"
          required
          fullWidth
          value={form.residenceBan.reasonLine}
          onChange={(v) => patch("residenceBan", "reasonLine", v)}
          rows={3}
        />
