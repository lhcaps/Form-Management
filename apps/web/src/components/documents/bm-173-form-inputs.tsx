...
const REQUIRED_FIELDS: RequiredField[] = [
  ...
  { section: "evidenceTransfer", field: "offenseName", label: "Tội danh" },
  { section: "evidenceTransfer", field: "caseTitle", label: "Tên vụ án" },
  { section: "evidenceTransfer", field: "evidenceListLine", label: "Vật chứng" },
...
function stripRecipient(value: string): string {
...
function buildCaseTitle(form: Bm173FormInputs): string {
  const explicit = form.evidenceTransfer.caseTitle.trim();
  if (explicit) return explicit;

  const offense = form.evidenceTransfer.offenseName.trim();
  const location = form.evidenceTransfer.caseLocation.trim();

  if (offense && location) return `Vụ án ${offense} tại ${location}`;
  if (offense) return `Vụ án ${offense}`;
...
function buildDerivedFields(form: Bm173FormInputs): Bm173FormInputs {
  ...
  const caseTitle = buildCaseTitle(form);
  const fromAgencyName = stripRecipient(form.evidenceTransfer.fromAgencyName);
  const toAgencyName = stripRecipient(form.evidenceTransfer.toAgencyName);
  ...
  const autoConsideration = ensureEnd(
    `Xét thấy cần phải chuyển các vật chứng của ${caseTitle} đến ${toAgencyName} để bảo đảm việc xét xử và thi hành án theo quy định của pháp luật`,
    ",",
  );

  const autoArticle1 = ensureEnd(
    `Chuyển các vật chứng sau đây ${evidenceListClean} từ ${fromAgencyName} đến ${toAgencyName}`,
    ".",
  ).replace(/([^\s])\.\s+từ\s+/giu, "$1 từ ");

  const autoArticle2 = ensureEnd(
  ...
  return {
    ...
    evidenceTransfer: {
      ...
      toAgencyName,
      prosecutionDecisionLegalBasisLine,
      considerationLine: firstText(form.evidenceTransfer.considerationLine, autoConsideration),
      article1Line: firstText(form.evidenceTransfer.article1Line, autoArticle1),
      article2Line: firstText(form.evidenceTransfer.article2Line, autoArticle2),
    },
    recipients: {
      ...form.recipients,
      line1: fromAgencyName ? `- ${fromAgencyName};` : "",
      line2: toAgencyName ? `- ${toAgencyName};` : "",
...
function normalizeFormInputs(payload: Record<string, unknown>): Bm173FormInputs {
  ...
  const sourceCaseTitle = pick(evidenceTransfer, "caseTitle");
  const sourceOffense = pick(evidenceTransfer, "offenseName");

  return buildDerivedFields({
    agency: {
      parentName: firstText(pick(agency, "parentName"), "VIỆN KIỂM SÁT NHÂN DÂN THÀNH PHỐ HỒ CHÍ MINH"),
      name: firstText(pick(agency, "name"), "VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7"),
    ...
    evidenceTransfer: {
      ...
      prosecutionDecisionLegalBasisLine: pick(evidenceTransfer, "prosecutionDecisionLegalBasisLine"),
      offenseName: firstText(sourceOffense, sourceCaseTitle.replace(/^Vụ án\s+/iu, "").replace(/\s+tại\s+.+$/iu, ""), "Đánh bạc"),
      caseLocation: firstText(pick(evidenceTransfer, "caseLocation"), "phường Trung Mỹ Tây"),
      caseTitle: firstText(sourceCaseTitle, ""),
      ...
      fromAgencyName: firstText(
        ...
        stripRecipient(pick(recipients, "line1")),
        "Cơ quan Cảnh sát điều tra Công an Thành phố Hồ Chí Minh",
      ),
      toAgencyName: firstText(
        pick(evidenceTransfer, "toAgencyName"),
        stripRecipient(pick(recipients, "line2")),
...
export function Bm173FormInputsPanel({ documentId, onSaved }: Props) {
  ...
  function updateField(sectionKey: SectionKey, field: string, value: string) {
    setForm((current) => {
      ...
      const fieldsThatRegenerateText = new Set([
        "offenseName",
        "caseLocation",
        "caseTitle",
        "evidenceListLine",
      ...
      if (sectionKey === "evidenceTransfer" && fieldsThatRegenerateText.has(field)) {
        next.evidenceTransfer = {
          ...next.evidenceTransfer,
          considerationLine: "",
          article1Line: "",
          article2Line: "",
      ...
      if (sectionKey === "evidenceTransfer" && field === "offenseName") {
        const location = next.evidenceTransfer.caseLocation || "phường Trung Mỹ Tây";
        next.evidenceTransfer = {
          ...next.evidenceTransfer,
          caseTitle: `Vụ án ${value} tại ${location}`,
          considerationLine: "",
          article1Line: "",
          article2Line: "",
        };
    ...
    setForm(
      buildDerivedFields({
        ...
        evidenceTransfer: {
          ...
          prosecutionDecisionLegalBasisLine: "",
          offenseName: "",
          caseLocation: "phường Trung Mỹ Tây",
  ...
  return (
    <div className="space-y-5">
      <BmFormCasePayloadButton templateCode="BM-173" form={form} onApply={(next) => setForm(next as typeof form)} />
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">BM-173</p>
            <h2 className="mt-1 text-xl font-bold text-slate-950">Quyết định chuyển vật chứng</h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
              Nhập dữ liệu chính. Tội danh chỉ nhập một lần, hệ thống tự đồng bộ tên vụ án, căn cứ, Điều 1, Điều 2 và nơi nhận.
            </p>
          </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
        <details open>
          ...
          <div className="mt-4 space-y-3 rounded-xl bg-white p-4 text-sm leading-6 text-slate-800">
            ...
            <p><strong>Nơi nhận:</strong> {derivedForm.recipients.line1} {derivedForm.recipients.line2}</p>
          </div>
        </details>
      </section>
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Sau khi lưu, render-payload phải đúng dữ liệu vừa nhập rồi mới xuất DOCX/PDF.
            </p>
          </div>
