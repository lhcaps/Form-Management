...
/**
 * BM-111 — QĐ hủy bỏ biện pháp tạm giữ
 * Stage: DIEU_TRA, Group: G04. TT 03/2026-VKSTC, Mẫu số 111/HS.
 *
...
function normalizeFormInputs(payload: RenderPayload | null): Bm111Form {
  ...
  return {
    ...
    decision: {
      ...
      oldDecisionDate: nested(payload, "decision.oldDecisionDate") || "",
      cancellationReason: nested(payload, "decision.cancellationReason") || "",
    },
    recipients: {
...
function lookupValue(form: Bm111Form, path: string): string {
  const parts = path.split(".").filter(Boolean);
  let cur: any = form;
  for (const p of parts) {
    if (!cur || typeof cur !== "object") return "";
...
function buildSaveBody(form: Bm111Form) {
  return {
    ...
    decision: {
      ...
      oldDecisionDate: form.decision.oldDecisionDate,
      cancellationReason: form.decision.cancellationReason,
...
export function Bm111FormInputsPanel({
  documentId,
  onSaved,
}: {
  ...
  const patch = <S extends keyof Bm111Form, K extends keyof Bm111Form[S]>(
    section: S,
    key: K,
    value: Bm111Form[S][K],
  ) => {
  ...
  return (
    <div className="space-y-5">
      <BmFormMetaBar
        templateCode="BM-111"
        title="Dữ liệu biểu mẫu QĐ hủy bỏ biện pháp tạm giữ"
        ...
        primaryLabel={saving ? "Đang lưu..." : "Lưu dữ liệu BM-111"}
      ...
      <BmFormSection title="2. Thông tin huỷ bỏ tạm giữ" description="Nhập thông tin QĐ tạm giữ bị huỷ bỏ." requiredCount={2}>
        <BmFieldText
        ...
        />
        <BmFieldDate
          label="Ngày QĐ tạm giữ"
          value={form.decision.oldDecisionDate}
          onChange={(v) => patch("decision", "oldDecisionDate", v)}
        />
        <BmFieldTextarea
          label="Lý do huỷ bỏ"
          required
          fullWidth
          value={form.decision.cancellationReason}
          onChange={(v) => patch("decision", "cancellationReason", v)}
          rows={3}
        />
        <BmFieldTextarea
          label="Lý do tự sinh"
