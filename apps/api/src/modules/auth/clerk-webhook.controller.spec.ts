...
function makeMockSvc(): { verifySignature: AnyMock; processEvent: AnyMock } {
  return {
...
describe('ClerkWebhookController', () => {
  describe('handleClerkWebhook', () => {
    ...
    it('returns 400 when rawBody is missing (no stringify fallback)', async () => {
      const ws = makeMockSvc();
    ...
    it('returns { ok: true } on valid signature and processed event', async () => {
      ...
      expect(ws.processEvent).toHaveBeenCalledWith({
        type: 'user.created',
        data: { id: 'user_123' },
      });
    });
