...
export class DbFormContractRepository extends FormContractRepository {
  ...
  private rowToLoaded(
    ...
    const genericFieldCount = version
      ? countGenericFields(version.compiled_json)
      : 0;
    ...
    const compiled = version?.compiled_json as
    ...
    return {
      ...
      canonicalFields: Array.isArray(source?.['fields'])
        ...
        : Array.isArray(draft?.['canonicalFields'])
          ? (draft['canonicalFields'] as Array<Record<string, unknown>>).map(
              (f) => ({
                path: String(f['path'] ?? ''),
      ...
      renderBindings: Array.isArray(source?.['renderBindings'])
        ? (
            source['renderBindings'] as Array<{
              id?: string;
              target?: { slotId?: string };
              source?: { fieldKey?: string };
              transform?: string;
              fallback?: unknown;
            }>
          ).map((b) => ({
        ...
        : Array.isArray(draft?.['renderBindings'])
          ? (draft['renderBindings'] as Array<Record<string, unknown>>).map(
              (b) => ({
