...
async function streamableFileToBuffer(file: StreamableFile): Promise<Buffer> {
  const chunks: Buffer[] = [];
...
describe('RuntimeTemplateRenderController', () => {
  describe('createPreviewSession', () => {
    it('returns RuntimePreviewSessionResponse JSON', async () => {
      const mockSession: RuntimePreviewSession = {
        ...
        fileFormat: 'DOCX',
        docxDownloadUrl:
          '/api/v1/forms/runtime/preview-sessions/runtime_preview_123e4567-e89b-12d3-a456-426614174000/docx',
        pdfPreviewUrl:
          '/api/v1/forms/runtime/preview-sessions/runtime_preview_123e4567-e89b-12d3-a456-426614174000/pdf',
        audit: {
          status: 'PASS',
          summary: {
            total: 18,
      ...
      const controller = new RuntimeTemplateRenderController(
        renderer as never,
        previewService as never,
      );

      const result = await controller.createPreviewSession('BM-001', {
  ...
  describe('downloadPreviewSessionDocx', () => {
    it('returns StreamableFile with DOCX content type', async () => {
      ...
      const previewService = {
        ...
        getSessionDocxPath: jest
          .fn()
          .mockResolvedValue('/fake/path/BM-001.docx'),
      };
      const renderer = { renderDocx: jest.fn() };
      const controller = new RuntimeTemplateRenderController(
        renderer as never,
  ...
  describe('getPreviewSessionPdf', () => {
    it('returns StreamableFile with PDF content type when available', async () => {
      ...
      const previewService = {
        getSession: jest.fn().mockResolvedValue(mockSession),
        getSessionPdfPath: jest
          .fn()
          .mockResolvedValue({
            pdfPath: '/fake/path/BM-001.pdf',
            available: true,
          }),
      };
      const renderer = { renderDocx: jest.fn() };
      ...
      expect(result).toBeInstanceOf(StreamableFile);
      const body = await streamableFileToBuffer(result);
    ...
    it('throws NotFoundException when PDF is not available', async () => {
      ...
      const previewService = {
        getSession: jest
          .fn()
          .mockResolvedValue({ sessionId: 's', fileName: 'f.docx' }),
        getSessionPdfPath: jest
          .fn()
          .mockResolvedValue({ pdfPath: '', available: false }),
      };
      const renderer = { renderDocx: jest.fn() };
      const controller = new RuntimeTemplateRenderController(
        renderer as never,
      ...
      await expect(
        controller.getPreviewSessionPdf(
          'runtime_preview_123e4567-e89b-12d3-a456-426614174000',
          mockRes,
        ),
      ).rejects.toThrow(NotFoundException);
