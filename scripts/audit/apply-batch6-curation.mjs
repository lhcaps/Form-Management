#!/usr/bin/env node
/**
 * Records Batch 6 source/render curation evidence.
 *
 * Batch 6 is source/render only. This script deliberately writes NOT_RUN /
...
const CANDIDATES = `${OUT_DIR}/QLLAW_BATCH6_CANDIDATES.latest.json`;
const ARTIFACT = `${OUT_DIR}/QLLAW_BATCH6_CURATION.latest.json`;
const ARTIFACT_MD = `${OUT_DIR}/QLLAW_BATCH6_CURATION.latest.md`;

export const BATCH6_CODES = [
  "BM-121",
  "BM-122",
  "BM-123",
  "BM-124",
...
const candidates = readJson(CANDIDATES, "Batch 6 candidates");
if (candidates.status !== "PASS") {
  fail(`candidate artifact status=${candidates.status}; expected PASS`);
}
if (JSON.stringify(candidates.selectedCodes) !== JSON.stringify(BATCH6_CODES)) {
  fail(
    `candidate codes drift: ${JSON.stringify(candidates.selectedCodes)}; expected ${JSON.stringify(BATCH6_CODES)}`,
  );
}
...
for (const code of BATCH6_CODES) {
  ...
  if (!row) fail(`missing Batch 6 row ${code}`);
  ...
  if (row.browserVerified === true) fail(`${code}: browserVerified=true; forbidden for Batch 6`);
  if (row.demoClickVerified === true) fail(`${code}: demoClickVerified=true; forbidden for Batch 6`);
  if (row.previewClickVerified === true) fail(`${code}: previewClickVerified=true; forbidden for Batch 6`);
  if (row.docxDownloadVerified === true) fail(`${code}: docxDownloadVerified=true; forbidden for Batch 6`);
  if (row.machineCheckableFidelityStatus === "PASS") fail(`${code}: machine fidelity PASS forbidden for Batch 6`);
  if (row.fidelityComplete === true) fail(`${code}: fidelityComplete=true; forbidden for Batch 6`);
...
// Accept either pre-Batch-7 baseline (117/96), post-Batch-7 state (137/76),
// post-Batch-8 state (157/56), or post-Batch-9 state (177/36). Each layer
// is idempotent and only writes per-form evidence for its own codes.
const allowedPassSet = new Set([117, 137, 157, 177]);
if (!allowedPassSet.has(passCount) || partialCount !== 213 - passCount) {
  fail(`count drift: INPUT_CONNECTED_PASS=${passCount}, INPUT_CONNECTED_PARTIAL=${partialCount}; expected 117/96 or 137/76 or 157/56 or 177/36`);
}

const snapshotDate = new Date().toISOString();
const batch6Set = new Set(BATCH6_CODES);
for (const row of rows) {
  if (!batch6Set.has(row.templateCode)) continue;
  ...
  row.batch6SourceRenderVerified = true;
}

matrix.snapshotDate = snapshotDate;
matrix.batch6CurationEvidence = {
  snapshotDate,
  ...
  totalForms: BATCH6_CODES.length,
  formsSourceRenderVerified: BATCH6_CODES.length,
  formsPass: BATCH6_CODES.length,
  formsFail: 0,
  existing97EvidencePreserved: true,
  formFlightRuntimeReadyPromoted: 0,
  codes: BATCH6_CODES,
  perForm: BATCH6_CODES.map((code) => {
    const row = byCode.get(code);
    return {
      code,
  ...
  filesStaged: false,
  notes: [
    "Batch 6 is source/render-only curation.",
    "Browser/demo/preview/DOCX/fidelity/visual/human evidence remains NOT_RUN for Batch 6.",
...
const artifact = {
  ...
  status: "PASS",
  statusNote:
    "All 20 Batch 6 forms were promoted from INPUT_CONNECTED_PARTIAL to INPUT_CONNECTED_PASS via source/render smoke only.",
  sourceRenderStatus: "PASS",
  browserVisibilityStatus: "NOT_RUN for Batch 6",
  demoClickStatus: "NOT_RUN for Batch 6",
  previewClickStatus: "NOT_RUN for Batch 6",
  docxDownloadStatus: "NOT_RUN for Batch 6",
  machineCheckableFidelityStatus: "NOT_RUN for Batch 6",
  visualPdfReviewStatus: "NOT_RUN for Batch 6",
  humanReviewStatus: "NOT_RUN for Batch 6",
  totalForms: BATCH6_CODES.length,
  formsSelected: BATCH6_CODES.length,
  formsCurated: BATCH6_CODES.length,
  formsSourceRenderVerified: BATCH6_CODES.length,
  ...
  codes: BATCH6_CODES,
  perForm: matrix.batch6CurationEvidence.perForm,
  refusals: {
    sourceDocxMutated: false,
    normalizedDocxMutated: false,
    lockedContractsMutated: false,
...
function renderArtifactMd(a) {
  const lines = [];
  lines.push("# QLLAW Batch 6 Curation - latest");
  lines.push("");
...
if (existsSync(MATRIX_MD)) {
  ...
  const sectionHeader = "## Batch 6 Curation Evidence";
  ...
  lines.push("- browserVisibilityStatus: NOT_RUN for Batch 6");
  lines.push("- demoClickStatus: NOT_RUN for Batch 6");
  lines.push("- previewClickStatus: NOT_RUN for Batch 6");
  lines.push("- docxDownloadStatus: NOT_RUN for Batch 6");
  lines.push("- machineCheckableFidelityStatus: NOT_RUN for Batch 6");
  lines.push("- visualPdfReviewStatus: NOT_RUN for Batch 6");
  lines.push("- humanReviewStatus: NOT_RUN for Batch 6");
  lines.push("- fidelityCompleteClaimed: false");
  lines.push(`- totalForms: ${BATCH6_CODES.length}`);
  lines.push(`- formsSourceRenderVerified: ${BATCH6_CODES.length}`);
  lines.push("- existing97EvidencePreserved: YES");
  lines.push("- formFlightRuntimeReadyPromoted: 0");
  lines.push("");
  lines.push("Artifact: `docs/audit/unified-bm-workspace/QLLAW_BATCH6_CURATION.latest.{md,json}`");
  lines.push("");
