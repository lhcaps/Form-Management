#!/usr/bin/env node
/**
 * Selects the Batch 8 source/render curation candidates.
 *
 * Read-only audit script. It does not promote evidence and does not mutate
 * DOCX/contracts/DB/schema. The selection rule is intentionally conservative:
 * numeric order after BM-160, exactly 20 INPUT_CONNECTED_PARTIAL rows if
 * available, skipping known special/skipped forms and forms missing any of
 * the hard gates.
 *
 * Batch 8 specifically excludes BM-171 (already INPUT_CONNECTED_PASS and
...
const CURATION = `${OUT_DIR}/QLLAW_BATCH8_CURATION.latest.json`;
const ARTIFACT = `${OUT_DIR}/QLLAW_BATCH8_CANDIDATES.latest.json`;
const ARTIFACT_MD = `${OUT_DIR}/QLLAW_BATCH8_CANDIDATES.latest.md`;
...
function renderMd(artifact) {
  const lines = [];
  lines.push("# QLLAW Batch 8 Candidates - latest");
  lines.push("");
...
if (existingCodes.length === TARGET_COUNT) {
  for (const code of existingCodes) {
    ...
    if (!candidate) fail(`existing Batch 8 curation code ${code} missing from matrix`);
    const reusable =
      candidate.currentMatrixStatus === "INPUT_CONNECTED_PARTIAL" ||
      candidate.currentMatrixStatus === "INPUT_CONNECTED_PASS";
    if (!reusable) {
      fail(`existing Batch 8 curation code ${code} has status=${candidate.currentMatrixStatus}; expected PARTIAL or PASS`);
    }
    selected.push({
      ...candidate,
      eligible: true,
      rejectionReason: null,
      selectionReason: "reused from QLLAW_BATCH8_CURATION.latest.json for idempotence",
    });
  }
}
  ...
  if (selected.length < TARGET_COUNT && candidate.eligible) {
    ...
    rejected.push({
      ...
      rejectionReason:
        ...
        `not selected after first ${TARGET_COUNT} eligible Batch 8 forms`,
    });
  }
}
...
const artifact = {
  ...
  selectionStrategy: [
    ...
    "Known special/skipped forms are rejected (BM-171 also excluded: already INPUT_CONNECTED_PASS pilot on the FormFlight runtimeReady allowlist).",
