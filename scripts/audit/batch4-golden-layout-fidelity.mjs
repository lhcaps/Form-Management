#!/usr/bin/env node
/**
 * batch4-golden-layout-fidelity.mjs
 *
 * Machine-checkable golden / layout fidelity audit for the 20 Batch 4
 * INPUT_CONNECTED_PASS forms (BM-076, BM-078, BM-080..BM-100).
 *
 * Mirrors the Batch 3 fidelity script (scripts/audit/batch3-golden-layout-fidelity.mjs)
 * but applies it to Batch 4 generated DOCX files emitted by
 * apply-batch4-docx-download.mjs.
 *
 * Reads:
 *   - docs/audit/unified-bm-workspace/QLLAW_BATCH4_DOCX_DOWNLOAD.latest.json (lifecycle evidence)
 *   - .tmp-batch4-docx-download-smoke/<code>.docx                        (generated runtime preview DOCX)
 ...
 *      - form title (templateTitle words) present
 ...
 *      - bold runs present
 *   E. Runtime lifecycle (from Batch 4 docx-download artifact)
 ...
 *   docs/audit/unified-bm-workspace/QLLAW_BATCH4_GOLDEN_LAYOUT_FIDELITY.latest.md
 *
...
const DOWNLOAD_ARTIFACT = `${OUT_DIR}/QLLAW_BATCH4_DOCX_DOWNLOAD.latest.json`;
...
// ---------------------------------------------------------------
// DOCX analysis helpers (mirrors batch3 + curated-37)
// ---------------------------------------------------------------
...
function hasRecipientsSlots(locked) {
  if (!locked || !locked.docxSlots) return false;
  const blockKeywords = [
    "nơi nhận", "noi nhan", "để biết", "de biet", "để thực hiện",
    "de thuc hien", "để chấp hành", "de cham hanh",
    "recipientblock", "distributionlist", "distribution_list",
  ];
  return locked.docxSlots.some((s) => {
...
function assessForm(code, downloadPerForm) {
  const result = {
    ...
    // E. Lifecycle (from Batch 4 docx-download artifact)
  ...
  if (downloadPerForm) {
    ...
    if (
      ...
      result.criteriaPassed.push("E.lifecycle_runtime_preview (proven by batch4 docx-download smoke)");
  ...
  if (!generatedZip) {
    result.failureReasons.push("Generated DOCX not found in .tmp-batch4-docx-download-smoke/");
  ...
  try {
    ...
    result.relsPresent = names.includes("_rels/.rels");
    ...
    if (result.documentXmlPresent) {
      const docXml = generatedZip.files["word/document.xml"].asText();
      result.documentXmlNonEmpty = docXml.trim().length > 0;
    }
    ...
    if (result.relsPresent) result.criteriaPassed.push("A.rels_present");
    else result.criteriaFailed.push("A.rels_present");
    ...
    if (result.documentXmlNonEmpty) result.criteriaPassed.push("A.document_xml_non_empty");
    else result.criteriaFailed.push("A.document_xml_non_empty");
  ...
  const criticalFails = result.criteriaFailed.filter((c) => {
    return /^A\.package_valid$/.test(c) ||
      ...
      /^A\.rels_present$/.test(c) ||
      /^A\.document_xml_present$/.test(c) ||
      /^A\.document_xml_non_empty$/.test(c) ||
...
function renderMarkdown(results, counts, snapshotDate) {
  const lines = [];
  lines.push("# QLLAW Batch 4 — Machine-Checkable Golden / Layout Fidelity Audit");
  lines.push("");
  ...
  lines.push("");
  lines.push("- **A** = Package / XML baseline (ZIP validity, content types, _rels, document.xml, styles, numbering)");
  ...
  lines.push("- Batch 4 visual/PDF review not run in this phase.");
...
async function main() {
  console.error("Starting batch 4 machine-checkable golden/layout fidelity audit...");
  ...
  if (!existsSync(DOWNLOAD_ARTIFACT)) {
    console.error(`FATAL: missing ${DOWNLOAD_ARTIFACT}. Re-run scripts/audit/apply-batch4-docx-download.mjs first.`);
    process.exit(2);
  }
  ...
  const summary = {
    ...
    visualPdfReviewStatusForBatch4: "NOT_RUN",
    counts,
    results,
    auditScript: "scripts/audit/batch4-golden-layout-fidelity.mjs",
    inputArtifacts: [
      "docs/audit/unified-bm-workspace/QLLAW_BATCH4_DOCX_DOWNLOAD.latest.json",
    ],
    sourcePaths: {
      generated: ".tmp-batch4-docx-download-smoke/<code>.docx",
    ...
    criteriaDef: {
      ...
      "A.rels_present": "_rels/.rels in package",
      "A.document_xml_present": "word/document.xml in package",
      "A.document_xml_non_empty": "word/document.xml has content",
    ...
    remainingRisks: [
      ...
      "Batch 4 visual/PDF review not run in this phase.",
  ...
  writeFileSync(
    `${OUT_DIR}/QLLAW_BATCH4_GOLDEN_LAYOUT_FIDELITY.latest.json`,
    JSON.stringify(summary, null, 2),
  );
  writeFileSync(
    `${OUT_DIR}/QLLAW_BATCH4_GOLDEN_LAYOUT_FIDELITY.latest.md`,
  ...
  console.error(`  ${OUT_DIR}/QLLAW_BATCH4_GOLDEN_LAYOUT_FIDELITY.latest.json`);
  console.error(`  ${OUT_DIR}/QLLAW_BATCH4_GOLDEN_LAYOUT_FIDELITY.latest.md`);
