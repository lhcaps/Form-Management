#!/usr/bin/env node
/**
 * Read-only guard for the 157-row source/render curation state (after Batch 8).
 *
 * Asserts:
 *   - 213 rows total.
 *   - INPUT_CONNECTED_PASS = 157.
 *   - INPUT_CONNECTED_PARTIAL = 56.
 ...
 *   - Batch 8 evidence: exactly 20 sourceRenderVerified, downstream NOT_RUN.
 ...
 *   - No selected Batch 8 row carries generated/workspace lifecycle fields.
 *   - No non-selected partial row got batch8SourceRenderVerified=true.
...
if (matrix.counts?.INPUT_CONNECTED_PASS !== 157 && matrix.counts?.INPUT_CONNECTED_PASS !== 177 && matrix.counts?.INPUT_CONNECTED_PASS !== 201) {
  fail(`counts.INPUT_CONNECTED_PASS=${matrix.counts?.INPUT_CONNECTED_PASS}; expected 157, 177, or 201`);
}
if (matrix.counts?.INPUT_CONNECTED_PARTIAL !== 56 && matrix.counts?.INPUT_CONNECTED_PARTIAL !== 36 && matrix.counts?.INPUT_CONNECTED_PARTIAL !== 12) {
  fail(`counts.INPUT_CONNECTED_PARTIAL=${matrix.counts?.INPUT_CONNECTED_PARTIAL}; expected 56, 36, or 12`);
}
  ...
  JSON.stringify(
    {
      ok: true,
      total: 213,
      inputConnectedPass: 157,
      inputConnectedPartial: 56,
      ...
      batch8SourceRenderVerified: BATCH8_CODES.length,
      batch8BrowserVerified: BATCH8_CODES.length,
      batch8DemoClickVerified: 0,
      batch8PreviewClickVerified: 0,
      batch8DocxDownloadVerified: 0,
      batch8MachineFidelityPass: 0,
      batch8VisualPdfReviewed: 0,
