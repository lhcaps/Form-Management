...
function hasAddressContext(text) {
  return /N[oơ]i\s+t[aạ]m\s+tr[uú]/iu.test(String(text || ''));
}

function hasRoleContext(text) {
  return /T[uư]\s+c[aá]ch\s+tham\s+gia\s+t[oố]\s+t[uụ]ng/iu.test(String(text || ''));
}

function classify(item) {
  const context = item.context || '';

  if (hasAddressContext(context) || hasRoleContext(context)) {
    return {
      classification: 'DEFER_PATH_DOMAIN_MISMATCH',
      reason: `${item.path} appears near an address/procedural-role label, but the current path/domain is generic or mismatched. This requires same-BM human DOCX/legal review before remap.`,
    };
  }
  ...
  if (item.suggestedPath || item.suggestedLabel) {
    return {
      ...
      reason: `${item.path} has an audit suggestion, but BM-136 evidence is still label/context-only and not enough for an approved mutation.`,
    };
  }
...
function main() {
  ...
  const evidence = {
    schemaVersion: '1.0.0',
    task: 'BM136_PATH_DOMAIN_BINDING_EVIDENCE',
    mode: 'EVIDENCE_ONLY',
  ...
  const patchPlan = {
    schemaVersion: '1.0.0',
    task: 'BM136_PATH_DOMAIN_BINDING_PATCH_PLAN',
    mode: 'EVIDENCE_ONLY',
    generatedAt: GENERATED_AT,
    templateCode: TEMPLATE_CODE,
    patchCandidates: [],
  ...
  const handoff = {
    ...
    nextPlannerDecision: 'Close BM-136 as BLOCKED_BY_HUMAN_DOCX_REVIEW or provide approved occurrence/path-level decisions for a separate apply task.',
  };

  const ledger = {
    task: 'BM136_REMAINING_PATH_DOMAIN_REVIEW_BLOCKER',
    schemaVersion: '1.0.0',
    templateCode: TEMPLATE_CODE,
    ...
    blockedPlaceholders: [
      {
        placeholder: 'BM-136.pathDomainGenericFields',
        count: deferred.length,
        classification: 'DEFER_REQUIRES_HUMAN_DOCX_REVIEW',
        reason: 'BM-136 render-fidelity passes, but remaining generic path-domain evidence is weak, placeholder-only, or domain-conflicting. Do not mutate without human DOCX/legal review.',
      },
    ],
    ...
    rejectedOptions: [
      {
        option: 'AUTO_REMAP_ALL_BM136_GENERIC_FIELDS',
        reason: 'The remaining evidence mixes document/person/agency/signature/recipient paths and several unlabeled adjacent placeholders. Batch mutation would be semantic guessing.',
      },
      {
        option: 'MARK_BM136_DONE_BECAUSE_RENDER_GATE_PASS',
    ...
    requiredHumanReviewQuestions: [
      'What official TT-03-2026-VKSTC label and semantic path corresponds to each BM-136 generic field?',
      'Which address-like occurrences should map to person/address payload paths rather than document paths?',
      'Which role/procedure occurrences should map to participant role fields rather than signature or document fields?',
      'Are adjacent person.field and document.field placeholders independent fields or split text from one legal phrase?',
    ],
    evidenceRefs: {
      renderDiff: 'docs/audit/per-form-render-accurate/BM-136/render-diff.latest.json',
      evidence: 'docs/audit/docx-placeholder-renormalization/BM-136/evidence.latest.json',
      patchPlan: 'docs/audit/docx-placeholder-renormalization/BM-136/patch-plan.latest.json',
      plannerHandoff: 'docs/audit/docx-placeholder-renormalization/BM-136/planner-handoff.latest.json',
    },
    createdAt: GENERATED_AT,
    createdBy: 'BM136_PATH_DOMAIN_BINDING_EVIDENCE task',
  };
  ...
  writeText('human-review-blocker.latest.md', [
    ...
    '## Render Gate',
    '',
    `Render gate is ${renderGate.status}. BM-136 is blocked for semantic/path-domain review, not render execution.`,
    '',
    '## Deferred Items',
    '',
  ...
  process.stdout.write(`[BM-136] evidence written to ${OUT_DIR}\n`);
  process.stdout.write(`[BM-136] candidates=${candidates.length} deferred=${deferred.length} render=${renderGate.status}\n`);
}
