#!/usr/bin/env node
/**
 * Selects the Batch 9 source/render curation candidates.
 *
 * Read-only audit script. It does not promote evidence and does not mutate
 * DOCX/contracts/DB/schema. The selection rule mirrors Batch 8:
 * numeric order after BM-180, exactly 20 INPUT_CONNECTED_PARTIAL rows if
 * available, skipping known special/skipped forms and forms missing any of
 * the hard gates.
 *
 * Batch 9 specifically excludes BM-171 (already INPUT_CONNECTED_PASS and
 * the FormFlight runtimeReady allowlist pilot) so we do not double-count or
 * mutate that allowlist state.
 */
...
const CURATION = `${OUT_DIR}/QLLAW_BATCH9_CURATION.latest.json`;
const ARTIFACT = `${OUT_DIR}/QLLAW_BATCH9_CANDIDATES.latest.json`;
const ARTIFACT_MD = `${OUT_DIR}/QLLAW_BATCH9_CANDIDATES.latest.md`;
...
// Batch 9 selection does NOT apply a hard "after BM-NNN" threshold.
// The matrix may end up with PARTIAL forms scattered across the full
// 213 range (e.g. legacy BM-002..BM-099 partials still pending). To
// produce a deterministic 20-form batch we pick the highest-numbered
// eligible PARTIAL forms first (closest to BM-213), then descend.
// BM-200 is excluded as the curated-runtime-ux-batch canary that must
...
function renderMd(artifact) {
  const lines = [];
  lines.push("# QLLAW Batch 9 Candidates - latest");
  lines.push("");
...
const inspected = rows
  .slice()
  .sort((a, b) => codeNumber(b.templateCode) - codeNumber(a.templateCode))
...
if (existingCodes.length === TARGET_COUNT) {
  for (const code of existingCodes) {
    ...
    if (!candidate) fail(`existing Batch 9 curation code ${code} missing from matrix`);
    const reusable =
      candidate.currentMatrixStatus === "INPUT_CONNECTED_PARTIAL" ||
      candidate.currentMatrixStatus === "INPUT_CONNECTED_PASS";
    if (!reusable) {
      fail(`existing Batch 9 curation code ${code} has status=${candidate.currentMatrixStatus}; expected PARTIAL or PASS`);
    }
    selected.push({
      ...candidate,
      eligible: true,
      rejectionReason: null,
      selectionReason: "reused from QLLAW_BATCH9_CURATION.latest.json for idempotence",
    });
  }
}
  ...
  if (selected.length < TARGET_COUNT && candidate.eligible) {
    ...
    rejected.push({
      ...
      rejectionReason:
        ...
        `not selected after first ${TARGET_COUNT} eligible Batch 9 forms`,
    });
  }
}
...
const artifact = {
  ...
  rejected,
  selectionStrategy: [
    `Numeric order (descending, highest first) so the batch picks the next 20 closest to BM-213.`,
    "Only INPUT_CONNECTED_PARTIAL rows are eligible.",
    "Known special/skipped forms and curated-runtime-ux-batch canaries (BM-200, BM-024, BM-130) are rejected. BM-171 is also excluded: already INPUT_CONNECTED_PASS pilot on the FormFlight runtimeReady allowlist.",
