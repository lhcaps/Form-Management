#!/usr/bin/env node
/**
 * batch3-visual-pdf-review.mjs
 *
 * Visual / PDF fidelity comparison for the 20 Batch 3 INPUT_CONNECTED_PASS
 * forms (BM-055..BM-069, BM-071..BM-075). Mirrors the Batch 4
 * visual/PDF review script (batch4-visual-pdf-review.mjs) but targets
 * the Batch 3 DOCX download smoke output and the Batch 3 machine-fidelity
 * artifact.
 *
 * Pipeline:
 ...
 *
 * Outputs:
 *   - .tmp-batch3-visual-pdf-review/<code>/source.pdf
 *   - .tmp-batch3-visual-pdf-review/<code>/generated.pdf
 *   - .tmp-batch3-visual-pdf-review/<code>/pages/{src,gen}_page_N.png
 *   - .tmp-batch3-visual-pdf-review/<code>/diff/diff_page_N.png
 *   - docs/audit/unified-bm-workspace/QLLAW_BATCH3_VISUAL_PDF_REVIEW.latest.json
 *   - docs/audit/unified-bm-workspace/QLLAW_BATCH3_VISUAL_PDF_REVIEW.latest.md
...
const TMP_DIR = `${ROOT}/.tmp-batch3-visual-pdf-review`;
...
// ---------------------------------------------------------------
// py.exe wrappers (same pattern as batch4 / curated-37)
// ---------------------------------------------------------------
...
function compareForm(code, fidelityByCode) {
  ...
  const result = {
    ...
    machineFidelitySource: fidelity ? "QLLAW_BATCH3_GOLDEN_LAYOUT_FIDELITY.latest.json" : null,
  ...
  if (result.machineFidelityStatus === "PASS") {
    result.notes.push("Batch 3 machine-checkable fidelity (golden/layout) PASS — see QLLAW_BATCH3_GOLDEN_LAYOUT_FIDELITY.latest.json.");
  }
...
function renderChecklistMd(results) {
  const lines = [];
  lines.push("# QLLAW Batch 3 — Visual / PDF Review Checklist");
  lines.push("");
  ...
  lines.push(">   - Source PDF: `.tmp-batch3-visual-pdf-review/<code>/source.pdf`");
  lines.push(">   - Generated PDF: `.tmp-batch3-visual-pdf-review/<code>/generated.pdf`");
  lines.push(">   - Page diff overlay: `.tmp-batch3-visual-pdf-review/<code>/diff/diff_page_N.png`");
  lines.push(">   - Per-page PNG: `.tmp-batch3-visual-pdf-review/<code>/pages/`");
  lines.push("");
  ...
  lines.push("");
  lines.push("1. Open the source PDF: `.tmp-batch3-visual-pdf-review/<code>/source.pdf`");
  lines.push("2. Open the generated PDF: `.tmp-batch3-visual-pdf-review/<code>/generated.pdf`");
...
function renderSummaryMd(results, counts) {
  const lines = [];
  lines.push("# QLLAW Batch 3 — Visual / PDF Fidelity Summary");
  lines.push("");
  lines.push(`> **Generated**: ${new Date().toISOString()}`);
  ...
  lines.push("- pdfplumber text extraction is unreliable for Vietnamese CJK fonts — DOCX XML text sanity was validated by QLLAW_BATCH3_GOLDEN_LAYOUT_FIDELITY.latest.json.");
...
async function main() {
  console.error("=== Batch 3 visual / PDF fidelity comparison ===");
  ...
  if (!existsSync(FIDELITY_ARTIFACT)) {
    console.error(`FATAL: missing ${FIDELITY_ARTIFACT}. Re-run scripts/audit/batch3-golden-layout-fidelity.mjs first.`);
    process.exit(2);
  }
  ...
  writeFileSync(
    `${OUT_DIR}/QLLAW_BATCH3_VISUAL_PDF_REVIEW_CHECKLIST.latest.md`,
    checklistMd,
  );

  const summaryMd = renderSummaryMd(results, counts);
  writeFileSync(
    `${OUT_DIR}/QLLAW_BATCH3_VISUAL_PDF_REVIEW.latest.md`,
    summaryMd,
  );
  ...
  const summary = {
    ...
    counts,
    results,
    auditScript: "scripts/audit/batch3-visual-pdf-review.mjs",
    toolInfo: {
      sofficePath: SOFFICE,
      pyExe: PY_EXE,
    ...
    batch3PriorEvidencePreserved: true,
    batch4EvidencePreserved: true,
  };

  writeFileSync(
    `${OUT_DIR}/QLLAW_BATCH3_VISUAL_PDF_REVIEW.latest.json`,
  ...
  console.error(`  ${OUT_DIR}/QLLAW_BATCH3_VISUAL_PDF_REVIEW.latest.json`);
  console.error(`  ${OUT_DIR}/QLLAW_BATCH3_VISUAL_PDF_REVIEW.latest.md`);
  console.error(`  ${OUT_DIR}/QLLAW_BATCH3_VISUAL_PDF_REVIEW_CHECKLIST.latest.md`);
