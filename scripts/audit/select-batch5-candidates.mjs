#!/usr/bin/env node
/**
 * Selects the Batch 5 source/render curation candidates.
 *
 ...
 * conservative: numeric order after BM-100, exactly 20 INPUT_CONNECTED_PARTIAL
 * rows if available, skipping known special/template-amendment forms.
 */
...
const CURATION = `${OUT_DIR}/QLLAW_BATCH5_CURATION.latest.json`;
const ARTIFACT = `${OUT_DIR}/QLLAW_BATCH5_CANDIDATES.latest.json`;
const ARTIFACT_MD = `${OUT_DIR}/QLLAW_BATCH5_CANDIDATES.latest.md`;
...
function inspectCandidate(row) {
  ...
  if (codeNumber(code) <= PREFERRED_AFTER) reasons.push("not after BM-100");
...
function renderMd(artifact) {
  const lines = [];
  lines.push("# QLLAW Batch 5 Candidates - latest");
  lines.push("");
...
if (existingCodes.length === TARGET_COUNT) {
  for (const code of existingCodes) {
    ...
    if (!candidate) fail(`existing Batch 5 curation code ${code} missing from matrix`);
    const reusable =
      candidate.currentMatrixStatus === "INPUT_CONNECTED_PARTIAL" ||
      candidate.currentMatrixStatus === "INPUT_CONNECTED_PASS";
    if (!reusable) {
      fail(`existing Batch 5 curation code ${code} has status=${candidate.currentMatrixStatus}; expected PARTIAL or PASS`);
    }
    selected.push({
      ...candidate,
      eligible: true,
      rejectionReason: null,
      selectionReason: "reused from QLLAW_BATCH5_CURATION.latest.json for idempotence",
    });
  }
}
  ...
  if (selected.length < TARGET_COUNT && candidate.eligible) {
    ...
    rejected.push({
      ...
      rejectionReason:
        ...
        `not selected after first ${TARGET_COUNT} eligible Batch 5 forms`,
    });
  }
}
