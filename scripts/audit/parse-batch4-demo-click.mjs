 ...
 *
 * Usage:
 *   node scripts/audit/parse-batch4-demo-click.mjs <input.json> <output.json>
 */
...
if (!inPath || !outPath) {
  console.error(
    "usage: parse-batch4-demo-click.mjs <input.json> <output.json>",
  );
  process.exit(2);
}

let raw = readFileSync(inPath);
// Detect UTF-16 LE BOM (FF FE) and decode. PowerShell's `>` operator writes
// text streams as UTF-16 LE on Windows; reading via utf8 mangles the file.
