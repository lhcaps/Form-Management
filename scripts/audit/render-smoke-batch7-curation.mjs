#!/usr/bin/env node
/**
 * Batch 7 source/render profile smoke.
 *
 * Read-only, local-file audit. It verifies the selected Batch 7 runtime UX
...
const OUT_JSON = `${OUT_DIR}/QLLAW_BATCH7_SOURCE_RENDER_SMOKE.latest.json`;
const OUT_MD = `${OUT_DIR}/QLLAW_BATCH7_SOURCE_RENDER_SMOKE.latest.md`;
...
const results = selectedCodes.map((code) => {
  ...
  const versionLabelCurated = profileSrc.includes(
    `${code} runtime-ux batch 7 curated source-render profile`,
  );
  const passes =
    normalized &&
    locked &&
...
const lines = [];
lines.push("# QLLAW Batch 7 Source/Render Smoke - latest");
lines.push("");
