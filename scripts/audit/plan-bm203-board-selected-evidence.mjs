...
function main() {
  ...
  const evidence = {
    schemaVersion: '1.0.0',
    task: 'BM203_BOARD_SELECTED_EVIDENCE_ONLY',
    mode: 'EVIDENCE_ONLY',
  ...
  const patchPlan = {
    schemaVersion: '1.0.0',
    task: 'BM203_SOURCE_POLICY_PATCH_PLAN',
    mode: 'EVIDENCE_ONLY',
    generatedAt: GENERATED_AT,
    templateCode: TEMPLATE_CODE,
    patchCandidates: [],
  ...
  const handoff = {
    ...
    nextPlannerDecision: 'Close BM-203 as BLOCKED_BY_HUMAN_DOCX_REVIEW or provide approved source/required/reviewRequired decisions for a separate apply task.',
  };

  const ledger = {
    task: 'BM203_SOURCE_POLICY_REVIEW_BLOCKER',
    schemaVersion: '1.0.0',
    templateCode: TEMPLATE_CODE,
    ...
    blockedPlaceholders: [
      {
        placeholder: 'BM-203.sourcePolicyFields',
        count: deferred.length,
        classification: 'DEFER_SOURCE_POLICY_REVIEW',
        reason: 'BM-203 render-fidelity passes, but source/required/reviewRequired policy findings are not apply-safe. Do not mutate locked metadata without human DOCX/legal/product review.',
      },
    ],
    ...
    rejectedOptions: [
      {
        option: 'AUTO_SET_REVIEW_REQUIRED_TRUE_FOR_ALL_BM203_FIELDS',
      ...
      {
        option: 'MARK_BM203_DONE_BECAUSE_RENDER_GATE_PASS',
    ...
    requiredHumanReviewQuestions: [
      ...
      'Should document.issueDate and document.fullDocumentCode be required for BM-203 runtime submission?',
      'Are any BM-203 recipient lines derived from case/participant data instead of manual input?',
      'Should BM-203 remain generic renderer or receive a bespoke authoring panel before metadata lock is considered complete?',
    ],
    evidenceRefs: {
      renderDiff: 'docs/audit/per-form-render-accurate/BM-203/render-diff.latest.json',
      evidence: 'docs/audit/docx-placeholder-renormalization/BM-203/evidence.latest.json',
      patchPlan: 'docs/audit/docx-placeholder-renormalization/BM-203/patch-plan.latest.json',
      plannerHandoff: 'docs/audit/docx-placeholder-renormalization/BM-203/planner-handoff.latest.json',
    },
    createdAt: GENERATED_AT,
    createdBy: 'BM203_BOARD_SELECTED_EVIDENCE_ONLY task',
  };
  ...
  writeText('patch-plan.latest.md', [
    '# BM-203 Source Policy Patch Plan',
    '',
    'Mode: EVIDENCE_ONLY',
    '',
    '- Patch candidates: 0',
  ...
  writeText('human-review-blocker.latest.md', [
    ...
    '## Render Gate',
    '',
    `Render gate is ${renderGate.status}. BM-203 is blocked for source-policy review, not render execution.`,
    '',
    '## Deferred Fields',
    '',
  ...
  writeText('live-state.latest.md', [
    '# BM-203 Live State',
    '',
    `**Template:** ${TEMPLATE_CODE} - ${evidence.title}`,
    `**SourceId:** ${evidence.sourceId}`,
    `**Primary lane:** SOURCE_POLICY`,
    `**Completion status:** NEEDS_REMEDIATION`,
    `**Root issues:** ${templateEntry.issueCount} (${templateEntry.failCount} FAIL / ${templateEntry.reviewCount} REVIEW)`,
    `**Render gate:** ${renderGate.status}`,
    '',
    '## Issue Summary',
    '',
    Object.entries(evidence.rootCause.issueCounts).map(([code, count]) => `- ${code}: ${count}`).join('\n'),
    '',
  ].join('\n'));

  process.stdout.write(`[BM-203] evidence written to ${OUT_DIR}\n`);
  process.stdout.write(`[BM-203] candidates=${candidates.length} deferred=${deferred.length} issues=${templateEntry.issueCount} render=${renderGate.status}\n`);
}
