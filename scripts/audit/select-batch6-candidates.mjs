#!/usr/bin/env node
/**
 * Selects the Batch 6 source/render curation candidates.
 *
 ...
 * conservative: numeric order after BM-120, exactly 20 INPUT_CONNECTED_PARTIAL
...
const CURATION = `${OUT_DIR}/QLLAW_BATCH6_CURATION.latest.json`;
const ARTIFACT = `${OUT_DIR}/QLLAW_BATCH6_CANDIDATES.latest.json`;
const ARTIFACT_MD = `${OUT_DIR}/QLLAW_BATCH6_CANDIDATES.latest.md`;
...
function renderMd(artifact) {
  const lines = [];
  lines.push("# QLLAW Batch 6 Candidates - latest");
  lines.push("");
...
if (existingCodes.length === TARGET_COUNT) {
  for (const code of existingCodes) {
    ...
    if (!candidate) fail(`existing Batch 6 curation code ${code} missing from matrix`);
    const reusable =
      candidate.currentMatrixStatus === "INPUT_CONNECTED_PARTIAL" ||
      candidate.currentMatrixStatus === "INPUT_CONNECTED_PASS";
    if (!reusable) {
      fail(`existing Batch 6 curation code ${code} has status=${candidate.currentMatrixStatus}; expected PARTIAL or PASS`);
    }
    selected.push({
      ...candidate,
      eligible: true,
      rejectionReason: null,
      selectionReason: "reused from QLLAW_BATCH6_CURATION.latest.json for idempotence",
    });
  }
}
  ...
  if (selected.length < TARGET_COUNT && candidate.eligible) {
    ...
    rejected.push({
      ...
      rejectionReason:
        ...
        `not selected after first ${TARGET_COUNT} eligible Batch 6 forms`,
    });
  }
}
