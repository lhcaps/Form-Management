#!/usr/bin/env node
/**
 * Read-only guard for the 137-row source/render curation state (after Batch 7).
 *
 * Asserts:
 *   - 213 rows total.
 *   - INPUT_CONNECTED_PASS = 137.
 *   - INPUT_CONNECTED_PARTIAL = 76.
 *   - curated 37 evidence intact.
 *   - Batch 3 evidence intact.
 *   - Batch 4 evidence intact.
 *   - Batch 5 evidence intact.
 *   - Batch 6 evidence intact.
 *   - Batch 7 evidence: exactly 20 sourceRenderVerified, downstream NOT_RUN.
 ...
 *   - No selected Batch 7 row carries generated/workspace lifecycle fields.
 *   - No non-selected partial row got batch7SourceRenderVerified=true.
...
if (matrix.counts?.INPUT_CONNECTED_PASS !== 137 && matrix.counts?.INPUT_CONNECTED_PASS !== 157 && matrix.counts?.INPUT_CONNECTED_PASS !== 177 && matrix.counts?.INPUT_CONNECTED_PASS !== 201) {
  fail(`counts.INPUT_CONNECTED_PASS=${matrix.counts?.INPUT_CONNECTED_PASS}; expected 137, 157, 177, or 201`);
}
if (matrix.counts?.INPUT_CONNECTED_PARTIAL !== 76 && matrix.counts?.INPUT_CONNECTED_PARTIAL !== 56 && matrix.counts?.INPUT_CONNECTED_PARTIAL !== 36 && matrix.counts?.INPUT_CONNECTED_PARTIAL !== 12) {
  fail(`counts.INPUT_CONNECTED_PARTIAL=${matrix.counts?.INPUT_CONNECTED_PARTIAL}; expected 76, 56, 36, or 12`);
}
...
if (batch7.sourceDocxMutated === true || batch7.refusals?.sourceDocxMutated === true) {
  fail("Batch 7 artifact claims source DOCX mutation");
}
if (batch7.normalizedDocxMutated === true || batch7.refusals?.normalizedDocxMutated === true) {
  fail("Batch 7 artifact claims normalized DOCX mutation");
}
if (batch7.lockedContractsMutated === true || batch7.refusals?.lockedContractsMutated === true) {
  fail("Batch 7 artifact claims locked contract mutation");
}
if (batch7.compiledContractsMutated === true || batch7.refusals?.compiledContractsMutated === true) {
  fail("Batch 7 artifact claims compiled contract mutation");
}
  ...
  JSON.stringify(
    {
      ok: true,
      total: 213,
      inputConnectedPass: 137,
      inputConnectedPartial: 76,
      ...
      batch7SourceRenderVerified: BATCH7_CODES.length,
      batch7BrowserVerified: BATCH7_CODES.length,
      batch7DemoClickVerified: 0,
      batch7PreviewClickVerified: 0,
      batch7DocxDownloadVerified: 0,
      batch7MachineFidelityPass: 0,
      batch7VisualPdfReviewed: 0,
