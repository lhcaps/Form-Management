#!/usr/bin/env node
/**
 * PR7A — BM-171 Visual Sign-off Packet Builder
 *
 * Goal: compose a single folder of evidence so a human/Planner can eyeball
 * the canonical rendered BM-171 DOCX and decide whether visual style
 * sign-off is granted.
 *
 * Pipeline:
 *   1. Shell out to apps/api/scripts/render-bm171-canonical-signoff.mjs which
 ...
 *        - BM-171 style profile engine
 *      Canonical fixture (PR7A) renders:
 ...
 *        - "ngày 04 tháng 7 năm 2026" (zero-stripped)
 ...
 *
 *   4. Emit the packet under docs/audit/bm-visual-signoff/BM-171/.
 *
 *   5. visualSignoffGranted is ALWAYS false from this script.
 *      rolloutReady is ALWAYS false from this script.
 *      Mass rollout / locked mutation never happen.
 *
 * Exit codes:
 ...
 * Refuses to write artefacts for BM-002..BM-213.
 *
 * Factory backlog: this script is a direct copy of
 * scripts/audit/build-bm001-visual-signoff-packet.mjs with BM-001-specific
 * canonical fixture values swapped for BM-171. The PR7B Form Rollout
 * Factory should collapse these into a single data-driven packet builder
 * (see docs/audit/unified-bm-workspace/PR7B_FORM_ROLLOUT_FACTORY_BACKLOG.latest.md
 * §F9).
 */
...
if (!TARGET) {
  console.error(
    '[FAIL] Missing target. Usage: node scripts/audit/build-bm171-visual-signoff-packet.mjs BM-171',
  );
  process.exit(2);
}
if (!ALLOWED_TARGETS.has(TARGET)) {
  console.error(
    `[FAIL] PR7A only prepares evidence for ${Array.from(
      ALLOWED_TARGETS,
...
const LOCKED_CONTRACT_PATH = join(
  ...
  'contracts',
  'locked',
  `${TARGET}__46b9a8be4e01.contract.locked.json`,
);
...
// ─── Canonical fixture (mirrors pr6g31-bm171-rendered-docx-parity.spec.ts) ──

const CANONICAL_REQUIRED_PRESENT = [
  // Header date line — `document.issuePlaceAndDateLine` slot at P0019.
  'TP. Hồ Chí Minh, ngày 04 tháng 07 năm 2026',
  // Body title — "QUYẾT ĐỊNH" (rendered as "QUYẾT ĐỊNH :")
  'QUYẾT ĐỊNH',
  // Subtitle — "TRẢ LẠI TÀI SẢN"
  'TRẢ LẠI TÀI SẢN',
  // Asset-owner date of birth — `assetOwner.dateOfBirthText` at P0036.
  'Sinh ngày 08/09/1985',
  // Asset-owner identity issued — `assetOwner.identityIssuedDateText` at P0040.
  'Cấp ngày 14/12/2021',
  // Archive line — `recipients.archiveLine` at P0049.
  'Lưu: HSVA, HSKS, VP.',
];

const CANONICAL_REQUIRED_ABSENT = [
  // BM-001 legacy archive wording.
  'Lưu: HSVV, VP.',
  ...
  '[object Object]',
  'Invalid Date',
  // PR7A.3 — drafter-note residue. PR7A.2 triage proved these are
  // plain body paragraphs (NOT real Word footnotes/endnotes) and
  // the locked contract does NOT bind P0080/P0081. The Planner
  // decision (Path B — MUST_SUPPRESS) routes them through the
  // BM-171 style profile's `dropParagraph` rules; if either leaks
  // back into the rendered DOCX, the suppression has regressed.
  // Both items MUST be absent for the packet to be
...
console.log(`[PASS] Scenario: canonical final-evidence fixture (PR7A)`);
...
const humanOnlyChecks = [
  ...
  {
    id: 'human-quoc-hieu-underline',
    label: 'Quốc hiệu underline width is visually correct',
    kind: 'human-only',
    status: 'NEEDS_HUMAN',
    evidence: 'visual underline width — open rendered.latest.docx in Word',
  },
  {
    id: 'human-thong-tu-size-13',
    label: '"Ban hành theo Thông tư…" remains 13pt (size 26)',
    kind: 'human-only',
    status: 'NEEDS_HUMAN',
    evidence: 'visual font size — open rendered.latest.docx in Word',
  },
  {
    id: 'human-title-bold-14',
    label: 'Title "QUYẾT ĐỊNH" bold + 14pt',
    kind: 'human-only',
    status: 'NEEDS_HUMAN',
  ...
  },
  {
    ...
    label: 'Subtitle "TRẢ LẠI TÀI SẢN" bold + 14pt',
    kind: 'human-only',
    status: 'NEEDS_HUMAN',
    evidence: 'visual weight + size — open rendered.latest.docx in Word',
  },
  {
    id: 'human-place-date-italic-14',
    label: 'Place-date line "TP. Hồ Chí Minh, ngày 04 tháng 07 năm 2026" italic + 14pt',
    kind: 'human-only',
    status: 'NEEDS_HUMAN',
    evidence: 'visual weight + size — open rendered.latest.docx in Word',
  },
  {
    id: 'human-article-1-bold-14',
    label: 'Heading "Điều 1." bold + 14pt',
    kind: 'human-only',
    status: 'NEEDS_HUMAN',
    evidence: 'visual weight + size — open rendered.latest.docx in Word',
  },
  {
    id: 'human-article-2-bold-14',
    label: 'Heading "Điều 2." bold + 14pt',
    kind: 'human-only',
    status: 'NEEDS_HUMAN',
    evidence: 'visual weight + size — open rendered.latest.docx in Word',
  },
  {
    id: 'human-noi-nhan-bold-14',
    label: '"Nơi nhận:" bold + 14pt',
    kind: 'human-only',
    status: 'NEEDS_HUMAN',
    evidence: 'visual weight + size — open rendered.latest.docx in Word',
  },
  {
    id: 'human-archive-line-11',
    label: 'Archive line "Lưu: HSVA, HSKS, VP." rendered at 11pt',
    kind: 'human-only',
    status: 'NEEDS_HUMAN',
    evidence: 'visual font size — open rendered.latest.docx in Word',
  },
...
// Structural presence checks (auto-confirmable from visible text)
const structuralPresenceChecks = [
  ...
  {
    id: 'structural-body-title',
    label: 'Body title "QUYẾT ĐỊNH" present',
    kind: 'auto-confirmable',
    status: /QUYẾT\s+ĐỊNH/u.test(visibleText) ? 'AUTO_OK' : 'FAIL',
    evidence: 'auto-detected from rendered DOCX visible text',
  },
  {
    id: 'structural-subtitle',
    label: 'Subtitle "TRẢ LẠI TÀI SẢN" present',
    kind: 'auto-confirmable',
    status: /TRẢ\s+LẠI\s+TÀI\s+SẢN/u.test(visibleText)
      ? 'AUTO_OK'
      : 'FAIL',
    evidence: 'auto-detected from rendered DOCX visible text',
  },
  {
    id: 'structural-article-1',
    label: 'Article heading "Điều 1." present',
    kind: 'auto-confirmable',
    status: /Điều\s*1\./u.test(visibleText) ? 'AUTO_OK' : 'FAIL',
    evidence: 'auto-detected from rendered DOCX visible text',
  },
  {
    id: 'structural-article-2',
    label: 'Article heading "Điều 2." present',
    kind: 'auto-confirmable',
    status: /Điều\s*2\./u.test(visibleText) ? 'AUTO_OK' : 'FAIL',
    evidence: 'auto-detected from rendered DOCX visible text',
  },
  {
    id: 'structural-noi-nhan',
    label: '"Nơi nhận:" present (BM-171 IS_APPLICABLE_BY_TEMPLATE)',
    kind: 'auto-confirmable',
    status: /Nơi\s+nhận\s*:/u.test(visibleText) ? 'AUTO_OK' : 'FAIL',
    evidence: 'auto-detected from rendered DOCX visible text (tolerates whitespace before colon)',
  },
];
...
const documentXmlInspection = {
  ...
  source: {
    ...
    canonicalFixture: 'PR7A (matches pr6g31-bm171-rendered-docx-parity.spec.ts)',
...
const visualSignoff = {
  ...
  packetProvenance: {
    renderPath: 'apps/api/scripts/render-bm171-canonical-signoff.mjs',
    canonicalFixture:
      'matches apps/api/src/.../pr6g31-bm171-rendered-docx-parity.spec.ts',
    renderedDocxSha256: renderResult.sha256,
    renderedDocxBytes: renderResult.byteLength,
    manifestPath: renderResult.manifestPath,
    shadowPath: renderResult.shadowPath,
  ...
  nextAction: packetStatus === 'READY_FOR_HUMAN_VISUAL_REVIEW'
    ? 'Open docs/audit/bm-visual-signoff/BM-171/rendered.latest.docx (or PDF/PNG if available) in Word. Walk the 12 NEEDS_HUMAN visual checks (bold/size/alignment/underline/page-numbers). Only Planner eyeball can flip visualSignoffGranted=true. After sign-off, re-run `pnpm audit:bm-rollout-ready -- BM-171`.'
...
md.push(`**rolloutReady**: false (PR7A rollout gate stays at BLOCKED_MANUAL_REVIEW)`);
md.push(`**Generated**: ${new Date().toISOString()}`);
md.push(`**Render path**: production (ContractRenderPlanBuilder + DocxtemplaterContractRenderEngine)`);
md.push(`**Canonical fixture**: PR7A — matches \`pr6g31-bm171-rendered-docx-parity.spec.ts\``);
...
md.push('- The PR7A rollout gate (`pnpm audit:bm-rollout-ready -- BM-171`) stays `BLOCKED_MANUAL_REVIEW` until a Planner eyeball confirms.');
