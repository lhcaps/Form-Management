#!/usr/bin/env node
/**
 * Batch 6 source/render profile smoke.
 *
 * Read-only, local-file audit. It verifies the selected Batch 6 runtime UX
...
const OUT_JSON = `${OUT_DIR}/QLLAW_BATCH6_SOURCE_RENDER_SMOKE.latest.json`;
const OUT_MD = `${OUT_DIR}/QLLAW_BATCH6_SOURCE_RENDER_SMOKE.latest.md`;
...
const results = selectedCodes.map((code) => {
  ...
  const versionLabelCurated = profileSrc.includes(
    `${code} runtime-ux batch 6 curated source-render profile`,
  );
  const passes =
    normalized &&
    locked &&
...
const lines = [];
lines.push("# QLLAW Batch 6 Source/Render Smoke - latest");
lines.push("");
