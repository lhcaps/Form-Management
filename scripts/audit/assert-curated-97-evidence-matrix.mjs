#!/usr/bin/env node
/**
 * Read-only guard for the 97-row source/render curation state.
 */
...
const artifact = readJson(BATCH5_ARTIFACT, "Batch 5 curation artifact");
const rows = matrix.rows ?? [];
if (matrix.total !== 213 || rows.length !== 213) {
  fail(`matrix.total=${matrix.total}, rows.length=${rows.length}; expected 213/213`);
}
if (matrix.counts?.INPUT_CONNECTED_PARTIAL !== 116 && matrix.counts?.INPUT_CONNECTED_PARTIAL !== 96 && matrix.counts?.INPUT_CONNECTED_PARTIAL !== 76 && matrix.counts?.INPUT_CONNECTED_PARTIAL !== 56 && matrix.counts?.INPUT_CONNECTED_PARTIAL !== 36 && matrix.counts?.INPUT_CONNECTED_PARTIAL !== 12) {
  fail(`counts.INPUT_CONNECTED_PARTIAL=${matrix.counts?.INPUT_CONNECTED_PARTIAL}; expected 116 (pre-Batch-6), 96 (post-Batch-6), 76 (post-Batch-7), 56 (post-Batch-8), 36 (post-Batch-9), or 12 (post-remaining-sweep)`);
}
const passCount = matrix.counts?.INPUT_CONNECTED_PASS;
const allowedPassSet = new Set([97, 117, 137, 157, 177, 201]);
if (!allowedPassSet.has(passCount)) {
  fail(`counts.INPUT_CONNECTED_PASS=${passCount}; expected one of [97, 117, 137, 157, 177, 201]. 97 = pre-Batch-6; 117 = post-Batch-6; 137 = post-Batch-7; 157 = post-Batch-8; 177 = post-Batch-9; 201 = post-remaining-sweep.`);
}

const byCode = new Map(rows.map((row) => [row.templateCode, row]));
for (const code of [...CURATED_37, ...BATCH3_CODES, ...BATCH4_CODES, ...BATCH5_CODES]) {
...
if (artifact.fidelityCompleteEvidenced !== false) {
  fail(`Batch 5 artifact fidelityCompleteEvidenced=${artifact.fidelityCompleteEvidenced}; expected false`);
}
...
if (artifact.sourceDocxMutated === true || artifact.refusals?.sourceDocxMutated === true) {
  fail("Batch 5 artifact claims source DOCX mutation");
}
if (artifact.normalizedDocxMutated === true || artifact.refusals?.normalizedDocxMutated === true) {
  fail("Batch 5 artifact claims normalized DOCX mutation");
}
if (artifact.lockedContractsMutated === true || artifact.refusals?.lockedContractsMutated === true) {
  fail("Batch 5 artifact claims locked contract mutation");
}
if (artifact.compiledContractsMutated === true || artifact.refusals?.compiledContractsMutated === true) {
  fail("Batch 5 artifact claims compiled contract mutation");
}
