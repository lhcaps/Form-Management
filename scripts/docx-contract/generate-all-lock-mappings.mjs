...
// Strip Vietnamese diacritics for ASCII comparison
...
function stripDiac(s) { return String(s).replace(DIAC_RE, (c) => DIAC_MAP[c] ?? c); }
...
// Try to resolve a generic field path (e.g. "document.field6") using form inputs
function resolveFromFormInputs(genericPath, formInputs, context) {
  if (!formInputs) return null;
  const m = genericPath.match(/^[a-z]+\.field(\d+)$/i);
  if (!m) return null;
  const idx = parseInt(m[1], 10) - 1;
  const fi = formInputs[idx];
  if (!fi) return null;
  // Build the field path using the form input's section + name
...
for (const code of ALL) {
  ...
  for (const slot of slots) {
    ...
    // Try to resolve generic IDs from form inputs
    if (/^[a-z]+\.field\d+$/i.test(slotId)) {
      const resolved = resolveFromFormInputs(slotId, formInputs, slot.evidence?.textBefore ?? slot.context ?? "");
  ...
  const fi = formInputs ? " FI" : "    ";
  console.log(`${tag}: ${code} (${slots.length} slots)${fi}`);
  count++;
}
