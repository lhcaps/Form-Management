/**
 * Authenticated DOCX download smoke for the 20 Batch 3 INPUT_CONNECTED_PASS
 * forms (BM-055..BM-069, BM-071..BM-075).
 *
 * Companion to:
 *   - tests/e2e/curated-batch3-preview-click.auth.spec.ts (preview-session POST)
 *   - tests/e2e/curated-batch3-demo-click.auth.spec.ts   (demo + editability)
 *   - tests/e2e/curated-37-docx-download.auth.spec.ts     (curated 37 ref)
 *
 * This spec clicks "Dữ liệu demo" + "Xem trước bản in" for every Batch 3 code,
 ...
 *     base64, then written under `.tmp-batch3-docx-download-smoke/`
...
const TMP_DOWNLOAD_DIR = resolve(
  process.cwd(),
  ".tmp-batch3-docx-download-smoke",
).replace(/\\/g, "/");
...
test.describe("Curated Batch 3 — authenticated DOCX download smoke", () => {
