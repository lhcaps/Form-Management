/**
 * Authenticated DOCX download smoke for the 20 Batch 4 INPUT_CONNECTED_PASS
 * forms (BM-076..BM-100).
 *
 * Companion to:
 *   - tests/e2e/curated-batch4-preview-click.auth.spec.ts (preview-session POST)
 ...
 *   - tests/e2e/curated-batch3-docx-download.auth.spec.ts (Batch 3 ref)
 *   - tests/e2e/curated-37-docx-download.auth.spec.ts     (curated 37 ref)
 *
 * This spec clicks "Dữ liệu demo" + "Xem trước bản in" for every Batch 4 code,
 ...
 *   8. No Content-Disposition attachment header on preview-session response.
 *   9. No navigation to /documents/.
 *  10. No "Lịch sử xử lý" link rendered in standalone.
 *  11. Authenticated GET against docxDownloadUrl returns 200 + DOCX content-type.
 *  12. Response buffer starts with "PK" (ZIP/DOCX magic).
 *  13. Buffer length > 5 KB.
 *  14. Buffer unzips and contains [Content_Types].xml + _rels/.rels + word/document.xml.
 *  15. word/document.xml text contains no "{{" / "}}" / "undefined" / "null"
 ...
 *     base64, then written under `.tmp-batch4-docx-download-smoke/`
 *     (gitignored) for ZIP / DOCX validation. No real download directory is
 *     touched, no `<a download>` element click is required, and there is
 *     zero browser-side flake.
 ...
 *   - DOCX_MISSING_RELS          — _rels/.rels not in package
...
const TMP_DOWNLOAD_DIR = resolve(
  process.cwd(),
  ".tmp-batch4-docx-download-smoke",
).replace(/\\/g, "/");
...
test.describe("Curated Batch 4 — authenticated DOCX download smoke", () => {
  for (const templateCode of BATCH4_CODES) {
    test(`${templateCode} downloads a real DOCX from runtime preview session`, async ({
      ...
      const previewContentDisposition =
      ...
      const previewSessionContentDispositionLeak =
        /attachment\s*;/i.test(previewContentDisposition);
      ...
      expect(
        previewSessionContentDispositionLeak,
        `${templateCode}: Content-Disposition attachment header present on preview-session response ("${previewContentDisposition}")`,
      ).toBe(false);
      expect(
