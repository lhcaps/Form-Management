# BM-171 Real Editor Verification

> Task: BM-171 Real Editor Vertical Slice + Legacy 60 Smoke
> Date: 2026-07-05
> Verdict: **PARTIAL** — DOCX is complete with all required structural elements;
> UI-driven `/documents/:id` round-trip on a real `generated_documents` row is the
...
One-line verdict: BM-171 form component + production render path produce a complete
DOCX for the canonical full synthetic fixture (34/34 slots, all required structural
elements present); a fresh UI-driven round-trip on a real `generated_documents`
row is recommended as the next step.
...
| Backend service / controller | `apps/api/src/modules/documents/document-renderer.controller.ts` + `apps/api/src/modules/documents/document-renderer.service.ts` + `apps/api/src/modules/documents/rendering/application/render-generated-document.use-case.ts` | Production render path |
...
No files in the main app source were modified for this task. The only file
changes are:

| File | Change | Reason | Risk |
|---|---|---|---|
| `scripts/audit/build-legacy-60-smoke-matrix.mjs` | new | Build the 60-form smoke matrix classification | none — read-only inspection of `bm-*-form-inputs.tsx` + `bm-panel-registry.generated.ts` |
| `docs/audit/legacy-60-stabilization/*` | new | Required artifact directory for the task | none |

`apps/api/scripts/render-bm171-canonical-signoff-full.mjs` is reused unchanged
(production render path with full synthetic fixture). The locked contract at
`docs/audit/docx/contracts/locked/BM-171__46b9a8be4e01.contract.locked.json` and
the normalized DOCX at `storage/templates/normalized-docx/BM-171/BM-171_normalized.docx`
are **not mutated**.
...
|---|---|---|
| editor loads | PASS (route wired; BM-171 component reached via `BM_PANEL_BY_CODE["BM-171"]`) | `apps/web/src/app/documents/[documentId]/page.tsx` → `GeneratedDocumentWorkspace` → `Panel = BM_PANEL_BY_CODE[templateCode]` → `Bm171FormInputsPanel` |
| form-specific BM-171 component used | PASS | `_registryWith172["BM-171"] = Bm171FormInputsPanel`; NOT `PublishedContractFormInputsPanel` / `GenericTemplateFormInputsPanel` |
...
| render blocked when required missing | PASS (workspace-level) | Workspace saves via `ContractFormInputsService.save` which validates `source.fields` and returns HTTP 422 + `issues[]` if any required slot is empty; render endpoint is only called via `pre-export-customization-panel` which the user must trigger after save |
| save form-inputs works | PASS (endpoint + helper) | `saveFormInputs(documentId, form)` → `saveDocumentFormInputs(documentId, body)` → `POST /documents/generated/:documentId/form-inputs` → `DocumentRendererController.updateFormInputs` → `DocumentRendererService.updateFormInputs` |
| DOCX generated | PASS | `render-bm171-canonical-signoff-full.mjs` exit 0; sha256 `4cf6f46a1c4c843db358813482f1fb28fd63b98c4a2db78c16d004a66cbf83c0`; 21557 bytes |
| PDF generated | NOT_AVAILABLE | this host lacks `soffice` / `pdftoppm`. PDF conversion is intentionally skipped. |
...
| legal basis distinct | PASS | 6 distinct "Căn cứ" lines (BLTTHS 134/212; truy tố 02/QĐ-VKS-KV7; tạm giam 15/QĐ-BCA; KLĐT 21/KLĐT-PCA; đình chỉ VA 03/QĐ-VKS-KV7; đình chỉ BC 04/QĐ-VKS-KV7) |
| Xét thấy visible | PASS | "Xét thấy tài sản bị tạm giữ không còn liên quan đến việc giải quyết vụ án và cần trả lại cho chủ sở hữu, quản lý hợp pháp theo quy định tại Điều 212 Bộ luật Tố tụng hình sự;" detected |
| asset lines visible | PASS | "01 chiếc xe máy Honda Wave RSX, biển số 59C1-123.45, màu đỏ-đen, năm sản xuất 2018, khung JL110E-1234567, máy JL110E-7654321;" + "01 sổ tiết kiệm Ngân hàng TMCP Ngoại thương Việt Nam, chi nhánh TP.HCM, số tài khoản 0011-2233-4455-66, số dư 12.500.000 đồng (Mười hai triệu năm trăm nghìn đồng);" detected |
...
| Điều 2 visible | PASS | "Điều 2. Yêu cầu Phòng Cảnh sát Quản lý hành chính về trật tự xã hội thuộc Công an Thành phố Hồ Chí Minh chuyển giao tài sản nêu tại Điều 1 cho ông Nguyễn Văn A trong thời hạn 05 ngày làm việc kể từ ngày nhận được Quyết định." detected |
...
| no undefined/null/Invalid Date | PASS | renderPlan.warnings=[], missingRequiredCount=0, no forbidden drift strings in rendered text |
| notes 12/13 absent | PASS | style-profile rules `bm171.drop_drafter_note_12` / `bm171.drop_drafter_note_13` (requireSuperscriptPrefix gated) dropped the notes |

## 5. Legacy 60 Smoke Matrix

See `LEGACY_60_SMOKE_MATRIX.latest.{md,json}` for full per-form classification.
...
|---|---|---|---|
| BM-002 | NEEDS_FIX | demo fixture contains bare `undefined`; no canonical `saveDocumentFormInputs` call | Replace fixture literal; migrate save path to `saveDocumentFormInputs` |
| BM-003..BM-018, BM-030, BM-031, BM-033, BM-037..BM-047, BM-053..BM-059, BM-070, BM-071, BM-086, BM-104, BM-141, BM-144..BM-148, BM-150, BM-156, BM-159, BM-166, BM-168..BM-170, BM-173 | NEEDS_FIX | no canonical `saveDocumentFormInputs` family call — each uses per-BM helper | Migrate each to canonical `api + options layer` (this is the "55 non-canonical forms" referenced in the task) |
| BM-172 | NEEDS_FIX | known adapter problem: `bm-172-form-inputs.tsx` exports `Bm172FormInputs` (not `Bm172FormInputsPanel`); workspace wraps via `_Bm172FormInputsPanelAdapter` | Decide whether to canonicalize the export shape (preferred) or keep the adapter |
...
| `pnpm --filter api exec tsx ./scripts/render-bm171-canonical-signoff-full.mjs` | 0 | DOCX rendered, sha256 `4cf6f46a…`, 21557 bytes, all 34/34 slots bound |
...
| BM171 verification MD | `docs/audit/legacy-60-stabilization/BM171_REAL_EDITOR_VERIFICATION.latest.md` (this file) |
...
| Stabilization report MD | `docs/audit/legacy-60-stabilization/LEGACY_60_STABILIZATION.latest.md` |
...
|---|---|---|
| DB has no BM-171 `generated_documents` row to drive an end-to-end UI round-trip in this session | Low | Seed or migrate a BM-171 generated document (PR7A.4 already exists; re-run with a real `caseId`) and exercise `/documents/:id` save → render from the browser. |
