...
**This is not a demo-data feature.** The current "Điền dữ liệu mẫu" button injects hardcoded demo values (fake people, fake dates in 2025-2026, fake offense descriptions). The goal is a separate "Điền nhanh thông tin chung" action that prefills only truly generic, context-independent fields.
...
| File | Role |
|------|------|
| `apps/web/src/features/forms-contracts/sample-data.ts` | Hardcoded demo sample data (excluded from hardcode audit) |
| `apps/web/src/components/documents/template-preview-workspace.tsx` | UI: "Điền dữ liệu mẫu" button → `applySampleData()` |
...
User clicks "Điền dữ liệu mẫu"
  → getSampleData(templateCode, fields)
    ...
    → generateSampleFromFields(fields) (heuristic generation)
  → mergeWithSampleData(existingData, sample)
    → for each sample field: if existing is empty → fill it
  → setData(merged)
  → message: "Đã điền dữ liệu mẫu vào các trường còn trống."
```
...
### 2.3 Where "TP. Hồ Chí Minh, ngày 01 tháng 01 năm 2026" Comes From

The stale 2026 place-date appears in:
- `DOCUMENT_DEFAULTS['document.issuePlaceDateLine']` = `"TP. Hồ Chí Minh, ngày 01 tháng 01 năm 2026"`
- `SAMPLE_REGISTRY['BM-001']['document.issuePlaceDateLine']` = same
- `D_DATE = "01"`, `D_MONTH = "01"`, `D_YEAR = "2026"`, `D_FULL_DATE = "2026-01-01"` (hardcoded constants)
- Label heuristic `if (l.includes("năm")) return "2026"` (generates "2026" for any field with "năm" in label)
...
**Option 1 (Recommended):** Rename existing button
```
OLD: "Điền dữ liệu mẫu"
NEW: "Điền nhanh thông tin chung"
HELPER: "Chỉ điền các trường chung như địa điểm, ngày lập và thông tin mặc định an toàn. Các thông tin quan trọng của vụ việc cần được nhập thủ công."
BEHAVIOR: Calls getSmartGenericPrefillData + mergeWithSmartPrefill (SAFE_RUNTIME_DEFAULT + SAFE_GENERIC_PREFILL only)
KEEP: "Điền dữ liệu mẫu" as separate action (or rename it to "Dữ liệu demo")
```

**Option 2:** Two separate buttons
```
Button 1: "Điền nhanh thông tin chung" → SAFE_RUNTIME_DEFAULT + SAFE_GENERIC_PREFILL
Button 2: "Dữ liệu demo" → current getSampleData behavior
```
...
4. **`sample-data.ts` coexisting**: The demo sample data still exists and still uses 2026 dates. Renaming/reorganizing it needs a separate decision.
...
| `docs/audit/smart-generic-prefill/latest.md` | TBD |
| `docs/audit/smart-generic-prefill/latest.json` | TBD |
| `apps/web/src/lib/smart-generic-prefill.ts` | NOT YET CREATED (implementation phase) |
